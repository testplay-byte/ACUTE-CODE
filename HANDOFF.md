...
**Last updated:** 2026-09-09 (round-80 DELIVERED — THE RELIABILITY ROUND, the owner's four-clause field report: the silent chat terminations fixed at three root causes — the SSE clean-close truncation guard in chat.ts, the context/request guard stops persisting honest turn.errors in runtime.ts, the route-crash persistence in server.ts; the raw error messages shown — every error-detail cap raised (500→4000 persisted, 240→600 user-facing, 500/160→4000/2000 connection-test); the retry customization — maxAttempts/waitMinutes/providerTimeoutSeconds resolved defensively per turn, edited from the Settings General card; the NVIDIA NIM provider — seeded, key-wired (Credential Manager + dev key file + env), nvapi- scrubbed, live-verified reachable+authenticated; see `docs/ui-iterations/round-80.md`): the session continued an IN-FLIGHT tree left by the context break — the implementation was verified claim-by-claim (lesson #82), one test-file typing bug and five stale-pin classes fixed honestly (the R58/R78 stream mocks now model the healthy wire — a finish-step part; the missing finish-step was exactly the silent-stop shape the guard exists to catch), and the whole stack re-proven: 2772/2772 in 149 files (agent-core 1796 incl. the four new r80 suites' 27 tests; frontend 964; e2e 12), lint clean, typechecks clean, and live on the real keys — the provider list seeds nvidia hasKey:true, the retry GET/PUT round-trips with honest 400s, the NVIDIA endpoint+catalog+key auth all work (the account's NIM model functions are EOL'd server-side — the raw bodies show verbatim), and a REAL OpenRouter streamed turn completes end-to-end ("R80 LIVE TURN OK") — the healthy-path proof. **R79 CLOSED: v0.78.0 = commit ef11d86 + close-out ede3455, tag v0.78.0 published, CI 34223271421 (one watched Windows-runner-stall re-run), release 384700391 live, DASHBOARD synced (8b8f84d).** The R80 release close-out (commit → tag v0.79.0 → release → CI watch → DASHBOARD truth-sync → status.json ci sync → ntfy) follows per MAINTENANCE recipe g. · **What's next:** the R81 queue: external plugin ctx enrichment (cline's appendContext seam), the lessons-ledger affordance as the reminder injector's third consumer, ratings-driven prompt tuning, the optional v0.68.0 backfill tag, and the standing items (edit-linting, installer code-signing, the Files-tab polish, agent web-app-testing tools) · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
...
## 3. Exact current state (2026-09-09, round-80 DELIVERED — v0.79.0 staged, release close-out pending)

**R80 is DELIVERED: the reliability round — the owner's four-clause field report (silent stops / raw errors / retry customization / NVIDIA) — shipped, unit-tested (2772 in 149 files: agent-core 1796 incl. four new r80 suites + frontend 964 + e2e 12), and live-verified on the real keys — see `docs/ui-iterations/round-80.md`.** R79 is CLOSED (v0.78.0 = commits `ef11d86` + close-out `ede3455`, tag v0.78.0 published, CI run 34223271421 green after one watched Windows-runner-stall re-run, release 384700391 live, DASHBOARD synced at `8b8f84d`). **The minimum commit to verify from is `ede3455`** — the R80 work (agent-core + src + tests + this round's docs + the 0.79.0 version bump) sits uncommitted ON TOP of it, with the pushed `backup/pre-r80` branch (ACUTE-CODE @ `ede3455`) as the recorded safety net; the release agent commits it, tags v0.79.0, pushes, and watches CI (status.json's ci field is the placeholder it syncs). The round-28-era table below is kept as the historical record.

**The next queue (R81+):** external plugin ctx enrichment (cline's appendContext seam), the lessons-ledger affordance as the reminder injector's third consumer, ratings-driven prompt tuning, the optional v0.68.0 backfill tag, and the standing items (edit-linting, installer code-signing, the Files-tab polish, agent web-app-testing tools). Known R80 residuals for the next agent: the NVIDIA account's NIM model functions are EOL'd server-side (nothing code-side can do — the raw 410 bodies surface honestly; pick models as NVIDIA re-provisions), and the sync-path child `turn.error` text stays the generic provider line (pre-existing, out of scope).
