...
**Last updated:** 2026-09-07 (round-71 DELIVERED — THE DISCIPLINE & RELIABILITY ROUND, research-driven from the four reference repos the owner supplied post-restore ("smarter better much more capable and reliable"): karpathy-skills + alirezarezvani/claude-skills (R71-b1), cline (R71-b2), kilocode (R71-b3); the R71-c audit scored the gap checklist 4 EXISTS / 12 PARTIAL / 13 MISSING and located nearly every gap in the model-facing layer — this round closes A1/A2/A4/A6/A7 (reliability), B1–B9 (discipline), C1/C2 (skill trigger surface); the round's code + docs are complete — NO tag, NO release, NO DASHBOARD sync: those are the orchestrator's close-out steps): (1) **THE ENGINEERING DISCIPLINE PROMPT SECTION** (NEW registry id, static/identity, composed after agentic-loop — prompts.ts:339-363, 2,312 chars): the four karpathy principles each with a binary self-test, [KNOWN]/[ASSUMED]/[UNKNOWN] tagging with "NEVER write code that depends on an [UNKNOWN]", THREE-STRIKE ESCALATION (3 failed fixes = STOP, re-diagnose, escalate), and a 5-line red-flags table quoting the model's own rationalizations; PLAN gained the task→verifiable-goal transform (prompts.ts:304); COMMUNICATION gained VERIFICATION RECEIPTS (exact command + exit status — "an assertion without a receipt is not verification"), 🟢🟡🔴 confidence tags, a devil's-advocate line, and anti-question-padding; SUB-AGENTS gained scope discipline + no-sleep/no-polling. Golden 20,156 → 23,665 bytes (sanctioned UPDATE_GOLDEN regen, diff audited line-by-line); the default full-tools composition 14,757 → 18,220 ≤ 22,000 (the 22K bound honestly re-anchored — the maximal now exceeds it by the mandated content floor, documented in the D6 test comment; one-time prompt-cache invalidation on first turn after upgrade). (2) **TOOL RELIABILITY (the e2 bundle)**: read_file truncation markers carry the file's TOTAL line count + the EXACT next call ("use offset=N to continue" — fs-ops.ts:307-313), the degenerate single-line cap says honestly no continuation can reach it and names real recovery tools; run_command's middle-omission marker teaches spill-to-file; NEW agent-core/src/tools/edit-streak.ts — the per-session consecutive edit-anchor-failure counter (2nd miss → re-read + copy the anchor EXACTLY; 3rd/4th → change approach or write_file wholesale; 5th+ → refuse the pattern; reset on success; in-memory only) wired at plugins/filesystem.ts:139-152; owner DENIALS now read "NOT a tool or system failure — ask why, or propose an alternative" with TIMEOUTS and aborts distinguished (approvals.ts:675-681/:939-945, computer-use consent too); classifyProviderError() (runtime.ts:707-852, exported) — six classes (context_window_exceeded/auth/rate_limit/network/timeout/unknown), auth BY STATUS ONLY, the rate-limit VETO before overflow patterns, bounded cycle-safe status walk, the class riding the 502 envelope + turn.error event additively; OVERFLOW RECOVERY on BOTH turn paths (streamed runtime.ts:1616-1643, sync :2380-2410): classified overflow with nothing streamed → visible "[context overflow → auto-compacted conversation → retrying]" meta.overflow_recovery SSE frame → forced compaction (compaction.ts force mode) → ONE retry; overflow twice = honest terminal line; never after partial content; non-overflow never compacts. (3) **THE SKILLS ROUND (8 → 12 builtins)**: all 8 descriptions rewritten trigger-rich ("Use when [quoted user phrasings]. Delivers [X]. NOT for [adjacent skill]" — 416-499 chars each; the description is the ONLY thing the model sees at trigger time) + four new discipline builtins at sortOrder 8-11 with the same INSERT OR IGNORE contract: focused-fix 1,858 chars (Iron Law NO FIXES WITHOUT COMPLETING SCOPE → TRACE → DIAGNOSE FIRST), zero-hallucination 1,751 ([KNOWN]/[ASSUMED]/[UNKNOWN] + the 6-rung YAGNI ladder), self-eval 1,776 (ambition×execution matrix, mandatory devil's advocate, a-claim-without-a-receipt-scores-0), ship-gate 1,680 (DO NOT SHIP / SHIP WITH NOTES / CLEAR). Honest limitation: description updates do NOT propagate to pre-R71 DBs (INSERT OR IGNORE keeps user-edited rows; fresh + revived rows get the new text). Verification: root **2287 tests in 129 files, all green** (2275 passed + 12 e2e env-gated skips without a fresh dist, re-run this docs pass; agent-core **1385/1385 in 66 files** — +110: r71-prompt-discipline 24 + r71-tool-reliability 31 + r71-skills-round 51 + re-pins r70-tool-feedback 18 / approval-flow 11 / r70-skills-system 41→45 / prompt-registry 22 / r70-prompt-round 34; frontend 890/890 unchanged — zero frontend files touched; typecheck + lint exit 0, re-run this pass), version 0.71.0 ×4, docs:check 168/0/0. Full detail in `docs/ui-iterations/round-71.md` · **What's next:** (a) the orchestrator's close-out (commit + CI + tag v0.71.0 + release + DASHBOARD truth-sync), then (b) the owner's live gates — the R71 questions (do receipts + confidence tags appear in real replies, do the four new skills trigger on real phrasings, does edit-escalation end the stale-anchor flail loops, does overflow recovery save long sessions) PLUS the R69/R70 gates still pending, then (c) the R72 queue: per-directory AGENTS.md injection, references/ for file skills, delegate_task task_id/background/resume, the generalized system-reminder injector, custom modes (.acute/agents/*.md), external plugin ctx enrichment · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
