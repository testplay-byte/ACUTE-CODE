...
**Last updated:** 2026-09-11 (round-92 DELIVERED: the fourth-walkthrough round, v0.89.0 → v0.90.0. THE MULTI-KEY POOL with automatic juggling (the headline): the owner's rework — sub-agents do NOT have separate keys; a provider holds Key 1..N added in Models & Providers' unified 'API keys' card (masked, reveal/copy, per-key test, the 31-key cap, the count chip on the provider rows); the Rust shell persists every provider's pool keys (store/remove_provider_key_slot + the generalized <id>-slot<N> credential targets + the ~/.acute/provider-pool-slots.txt note file — restart survival for EVERY provider, and the R47 bug where adding a pool key in the packaged app silently OVERWROTE the primary is fixed); the turn runners JUGGLE — on auth (key rejected) or rate_limit (that key's quota spent) with an untried key left, the SAME call retries on the next key IMMEDIATELY (no ladder wait; the streamed runner flushes the partial segment first; meta.key renders 'switching to API key 2 of 3' in the live chat; pool exhaustion hands to the R75 ladder); sub-agents ride the same pool (the orchestrator's single-key child views deleted; the subagents tab keeps the model picker/parallelism/supervision and links to the keys' one home). THE BROWSER-EMBEDDING FINISH (the 'I still don't feel like the browser is an embedded part' verdict): the composer's mode/thinking/add-context menus ride the menu-overlay window ABOVE the browser (useNativeOptionsMenu — the R90-C2 ladder generalized; the DOM dropdown stays the web/fallback leg); pure backdrops are exempt from the webview guard (the model picker's fixed inset-0 dim was the deterministic 'cleared out'); the guard self-heals (zero-rect skip + rAF retry, the 600ms periodic re-sweep, the watchdog's fresh sweep before its geometric skip); the paused caption renders on both hide legs. THE GENERATION DEAD END closed at both ends: prepareTurn's gate now checks the EFFECTIVE pair (the send's override satisfies it on a NULL/reset agent) and the chat picker's first pick ARMS an unconfigured agent row (the wizard's PATCH precedent) — 'agent acute has no provider ID/model configured' cannot re-appear. AGENTS EDITABLE AGAIN: the NULL-model form crash fixed (model/provider optional with the 'leave empty to pick the model in chat' hint, the '— none —' provider option, explicit-null PATCH semantics, the shared AgentRecord types telling the backend truth). Full pipeline green: root 2,953/2,953 (165 files) + e2e 12/12 + license 134 clean + windows-target cargo check + docs:check 197/0/0. The owner's TEST CHECKLIST lives in round-92.md §8.) · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
