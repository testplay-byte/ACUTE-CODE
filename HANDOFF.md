...
**Last updated:** 2026-09-07 (round-76 DELIVERED — THE DOCUMENTATION ROUND, the owner's ask: "update the documentation… everything up to date and well managed… accurate and properly structured"; docs-only — no code, no version bump, no release): the audit (two research agents — R76-a the 20-item staleness sweep verified against code, R76-b the backfill) found the truth docs had drifted behind the code, and every gap is closed: ORCHESTRATION-WORKLOG (the session history this file's §1 point 4 sends every new agent to) had STOPPED AT R67 — R68–R75 backfilled (8 entries, pure append +147/−0, source conflicts resolved in git's favor, the v0.68.0-tag gap and R69's lost run ids recorded honestly); AGENT-MEMORY had stopped at lesson #71/R58 — five lessons appended (#72 drafts-above-published ordering, #73 the SDK's non-throw error-PART channel, #74 flex-1 eating inline heights, #75 the stamp-cohort aging, #76 "complete but uncommitted" is a claim); ARCHITECTURE §2.4 described a shutdown route that never existed + the pre-R75 terminal-failed sweep (rewritten to the INTERRUPTED/queued truth; the module-map note now names the R75 modules and says the WS gateway was never built — SSE is the shipped surface); IMPLEMENTED-API refreshed R73→R75 (26 tools, /modes readOnly, the meta.retry frame, attempts, the ROUND-75 section); EXTENSIBILITY carries the R75 enforcement addendum (mode-policy intersections, owner-pinned switch_mode, delegation inheritance, custom∩policy); DESIGN-SYSTEM's composer anatomy was pre-R50 (rewritten: the one-row @container + the 5-line autogrow + the retry/error cards); ROADMAP unfrozen from round-17; SECURITY control 7 (the task-mode enforcement tier); MAINTENANCE recipe g step 2b + DOC-STANDARDS §8 (the stamp-cohort rule: a 100+-doc docs:check failure is cohort aging, not rot — stamp-all only ADDS stamps); the docs index gained five unindexed runbooks (DOC-STANDARDS, ORCHESTRATOR-METHOD, REVIEW-CADENCE, PROMPT-MODULES, CLI-HARNESS); NEW ADR-0026 (task-mode tool policy — hard enforcement with owner pinning) + NEW ADR-0027 (the retry-ladder's three-layer composition) — the two load-bearing R75 decisions had no record; HANDOFF/AGENTS/SPEC/README corrected (REST+SSE, the glm-5.2:free model truth, ADRs 0001–0027, §3 = round-75 CLOSED with receipts, `66e949a`, the verify chain incl. test:e2e); status.json round 76 + milestone 36 + the R76 queue. **R75 CLOSED END-TO-END BEFORE THIS ROUND: v0.75.0 published (release 384282923, commit 66e949a, CI run 34154269987 green on first push, Release run 34154773400; setup.exe 37,395,634 B + launcher-kit 88,402 B, both sha256-digested; the live picker smoke returned 0.75.0/22 releases; DASHBOARD truth-synced e51c053→c1e7039, Pages live) — the owner's four verbatim asks (the transient-API retry ladder, hard task-mode enforcement, hover-only mode menus, the one-row composer + 5-line textarea) shipped and live-verified; full detail in docs/ui-iterations/round-75.md.** Verification this round: docs:check 173/0/0; the 2613-test suite/lint/typechecks unchanged by construction (no source files touched — CI re-runs them on the push); status.json JSON.parse-gated; the worklog backfill diff-verified pure-append. · **What's next:** (a) the owner's R75 live gates (plan mode refusing real writes, the retry card on a real rate limit, the one-row toolbar, the 5-line textarea), then (b) the R76 queue: delegate_task task_id/background/resume (the standing R73 deferral), external plugin ctx enrichment (cline's appendContext seam), lessons-ledger as the reminder injector's third consumer, ratings-driven prompt tuning, the optional v0.68.0 backfill · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
...
## 3. Exact current state (2026-09-07, round-75 CLOSED — v0.75.0 released)

**R75 is CLOSED: v0.75.0 published (release 384282923, commit 66e949a, CI run 34154269987 green on first push); root 2613 tests green in 140 files; docs:check 173/0/0. The owner's four asks (the transient-API retry ladder, hard task-mode enforcement, hover-only mode menus, the one-row composer + 5-line textarea) are shipped and live-verified — see `docs/ui-iterations/round-75.md`.** The round-28-era table below is kept as the historical record.
