...
**Last updated:** 2026-08-30 (round-54 DELIVERED — the reliability round; every fix traces to the owner's seventh Windows test session — the SECOND run of the packaged app, which updated to 0.53.0 and still failed): **THE LAUNCHER'S REGISTRY-TRUST BUG** (the owner's console showed `installed desktop app 0.53.0 is current` and then `ACUTE-CODE.exe not found — using the dev-servers flow` after they deleted the app folder: the NSIS uninstall entry SURVIVES folder deletion, so the launcher skipped the install and fell back to the browser — the install is now verified ON DISK: exe + pinned node.exe + the sidecar entry, both tauri resource layouts probed; a broken install is REPAIRED by reinstalling, and the console prints exactly what was missing); **THE PACKAGED APP'S BLIND STDERR** (agent-core prints its real startup crash to stderr — but a GUI-subsystem process has NO stderr handle, so `Stdio::inherit()` sent it nowhere and every failure could only ever read "stdout closed before the ready line"; stderr is now PIPED and drained into sidecar.log as `sidecar:stderr] …` plus an in-memory ring, and the Failed-phase error embeds the engine's recent output); **THE ORPHANED CHILD** (a failed handshake used to DROP the Child handle — on Windows the node.exe kept running, holding the SQLite DB while every Restart-engine attempt raced a zombie: the actual mechanism behind the owner's "restart engine did not work"; failed attempts now kill_tree + wait their child); **RETRIES + BIGGER COLD-BOOT BUDGETS** (the handshake auto-retries 3× with 25s ready/15s health deadlines — Defender scanning a fresh 216 MB install is exactly the launch that outran the old 15s one-shot; the UI connect deadline follows: 90s → 150s); **IN-APP ENGINE DIAGNOSTICS** (the offline screen now renders the last 60 sidecar.log lines in a scrollable box via the NEW `sidecar_log_tail` shell command, with a Copy-diagnostics button that puts error + log on the clipboard — no more "check %APPDATA% by hand"); **THE LAUNCHER WATCHES THE ENGINE** (after launching the app, ACUTE.bat polls sidecar.log for `listening on 127.0.0.1:<port>` and prints `✓ agent-core is up — port N`, or the failure log tail — the launcher console now tells the same story the app window does; running instances are also closed before install/launch, precisely by install-dir path so only the app + its bundled node die); **BOUNDED BROWSER FOLDER PICKING** (the Add-project dialog's Browse fetch aborts after 2 minutes — a hung OS dialog used to spin forever — with immediate "Opening the system folder dialog…" feedback and every failure hint ending in "paste the folder path above instead"). Verification: cargo check GREEN on x86_64-pc-windows-gnu, the launcher helpers logic-tested against the owner's EXACT scenario (registry-only → reinstall; PS `-like` pattern byte-checked; watch-engine success/failure/rotation/timeout paths), +12 frontend tests (ConnectionGate.test.tsx NEW), live browser battery. Version **0.54.0**. Full detail in `docs/ui-iterations/round-54.md` · **What's next:** the owner's Windows re-test (double-click ACUTE.bat → auto-upgrade to 0.54.0 → the launcher now reinstalls broken installs, watches the engine boot, and the app's offline screen self-explains), installer code-signing (SmartScreen), the deepseek-harness future candidates, Files-tab polish · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
