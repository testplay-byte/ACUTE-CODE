...
**Last updated:** 2026-09-07 (round-75 DELIVERED — THE RELIABILITY & ENFORCEMENT ROUND, the owner's four verbatim asks from the 0.74.0 field report, every one browser-verified live before shipping; the code + tests + docs are complete — the close-out (commit + CI + tag v0.75.0 + release + DASHBOARD truth-sync) is the orchestrator's next step): (1) **THE TRANSIENT-API RETRY LADDER** (NEW `agent-core/src/lib/retry.ts`; the owner's schedule verbatim in `RETRY_LADDER_MS` = [immediate, 1.5 min, 5 min, 10 min, 30 min] — six attempts total, then stop + notify + show the error): rate_limit/network/timeout failures ladder; auth/overflow/unknown NEVER auto-retry (fail fast, the owner's "other kind of issue" rule); the wait is abort-aware (a user Stop cuts it short → the honest ABORTED path), ticks a 60 s SSE heartbeat (keep-alive + live countdown), and re-runs the SAME outer-loop iteration (`outerIter` compensated — the ladder never spends loop budget, bounded by the rung count). The R71 classifier moved to NEW pure `agents/error-classification.ts` (runtime re-exports; the transient split `isTransientApiFailure` joins it). THE DEEPEST FIND OF THE ROUND: the AI SDK delivers post-retry provider errors as fullStream ERROR PARTS, which `streamAiSdkChat` ignored — the runtime only ever saw the generic "No output generated" rejection → class unknown → the ladder could NEVER engage on real 429s and the cards showed no honest class; the adapter now re-throws the original error (chat.ts), LIVE-VERIFIED against a mock 429 provider: meta.retry frames on the wire (attempt 2 immediate → attempt 3 at 90 s with heartbeat ticks) + `provider.retry_ladder` log lines. The UI: NEW RetryStatusCard (amber role=status — alive, not alarming: "Retrying — attempt 2 of 6", the class chip, the class message, a per-second countdown, "the agent keeps working automatically"), rendered above everything while LiveTurn.retry is set and CLEARED by the first content frame; the terminal TurnErrorCard gains the class chip + "after 6 attempts"; the task_failed notification body says "auto-retried 6 times (immediate, 1.5, 5, 10, 30 min)". (2) **TASK MODES ARE NOW HARD-ENFORCED** (NEW pure `agents/mode-policy.ts`): the root cause of "plan mode tried to make edits" was that the R73 task modes were PROMPT-ONLY — the prose said NO EDITS while the toolset still carried write_file/edit_file/run_command. Now plan/review/explore INTERSECT the turn's allowlist to read-only sets (plan = PLAN_MODE_TOOLS verbatim — moved to mode-policy.ts, runtime re-exports; review/explore add the git inspectors + analyze_image + job_status; switch_mode stays as the escape hatch), debug keeps the full toolset but run_command demotes to the AUTO tier in approvals.ts (ask-tier DENIED with an honest note even under Full-Access — the task mode outranks the widening), build/refactor pass through, custom frontmatter ∩ policy (a custom shadow of a read-only builtin stays read-only), and the prompt's toolNames matches the narrowed set exactly (the model never sees a tool it cannot call). Delegated children COPY the parent's activeMode (createSession gained the input; the INSERT only includes the column when set — pre-0027 schemas stay valid). READ-ONLY MODES ARE OWNER-PINNED: the first live e2e showed the model discovering it had no write tools and calling switch_mode("build") to ESCAPE — switch_mode now refuses to leave/clear plan/review/explore ("a read-only posture the OWNER set — ask the owner to switch"); every non-read-only switch is unchanged. LIVE-VERIFIED: a plan-mode session asked to create a file honestly reported its read-only toolset and NO file was written. The mode bodies now carry the fact ("This is ENFORCED, not advisory (R75): … REMOVED from your toolset — you literally cannot call them"). (3) **Mode menus are single-line** (the owner: descriptions "should not be shown by default; only when the user hovers"): TaskModePicker + ModeSwitcher rows render name-only, the description rides the row's native title tooltip, and plan/review/explore rows carry an accent "read-only" badge (the /modes route gained `readOnly` from the policy so the frontend never duplicates the set). (4) **THE COMPOSER**: ONE row in the owner's exact order (attach, access, mode, context, model, reasoning, send) — no left/right clusters, no justify-between; the box is a CSS @container and the four selector pills' labels hide below 560px (icon-only; tooltips carry them) so the row fits at the 480px chat floor (measured 44 px one-row at 386–641 px widths; the old design wrapped to 76 px/two rows below ~530 px); flex-wrap survives as the R51-c never-overlap fallback. THE TEXTAREA NEVER AUTO-GREW (proven live: the JS set height:132px while clientHeight stayed 34px — the textarea's `flex-1` (flex-basis 0%) made the flex algorithm IGNORE the height style, the owner's "cut off to only two lines"): `flex-1` is GONE and the growth lives in a useLayoutEffect keyed on `input` — fires on typing AND programmatic fills (suggestion chips), caps at exactly 5 visible lines derived from the LIVE computed line-height + paddings (maxHeight set alongside so CSS/JS never disagree), then scrolls internally (live-verified: 3 lines → 73 px, empty → 34 px, 8 lines → 112 px capped). ALSO FIXED THIS ROUND: the sync-path ok:true lie (a sub-agent failing mid-work after partial replies used to persist turn.error but return success — the orchestrator marked it completed and the parent built on half-done work; now records the completed iterations' usage, persists, returns honest 502), the boot sweep's silent death (a sidecar crash mid-turn flipped the session to terminal failed with NOTHING in the timeline and 409'd the next send; now writes an INTERRUPTED turn.error event + resets to queued — the interruption is visible forever), the terminal error path now flushes partial streamed text + records usage (the R58-c rule, extended to errors), errorClass+attempts ride 502 details → TurnErrorInfo → ErrorTurnItem → the card, and meta.overflow_recovery finally renders (it was untyped fall-through). Supervisor interplay: the R52-b stall watchdog consults the ladder's active-wait registry — a child waiting out a 30-min rung is alive-by-construction (the heartbeat frame says "waiting to retry (attempt 3/6, Ns remaining)"), never stall-killed. Verification: root vitest **2613 green (2558 + 55)** in 140 files — agent-core 1685 (76 files; +48: NEW r75-retry-ladder 19 + NEW r75-mode-policy 27 incl. the pinning refusals) + frontend 916 (62 files; +7: the single-row toolbar contract, hover-only + badge contracts, RetryStatusCard + retry-frame + enriched-error tests) + 12 e2e env-gated; chat-format +2 (the error-PART re-throw pair); lint + both typechecks + docs:check (172/0/0) clean; LIVE browser verification of all four features (the plan-mode e2e with a real model, the mock-429 ladder on the wire, the toolbar at 5 widths, the textarea growth matrix). Full detail in docs/ui-iterations/round-75.md · **What's next:** (a) the orchestrator's close-out (commit + CI + tag v0.75.0 + release + DASHBOARD truth-sync — milestone 56 + feature card 117), then (b) the owner's live gates (plan mode refusing writes, the retry card on a real rate limit, the one-row toolbar, the 5-line textarea), then (c) the R76 queue: delegate_task task_id/background/resume (the standing R73 deferral), external plugin ctx enrichment (cline's appendContext seam), lessons-ledger as the reminder injector's third consumer, ratings-driven prompt tuning, the optional v0.68.0 backfill · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
