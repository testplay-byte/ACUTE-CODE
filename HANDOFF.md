...
**Last updated:** 2026-09-02 (round-65 DELIVERED — the honesty patch, closing the four R64 leftovers from the owner's live-run feedback): (1) **THE SURFACE BOUNDARY** — the 0.63.0 hallucination root cause closed at the prompt layer: BOTH the computer-use and the embedded-browser sections now carry an explicit R65 boundary line naming the OTHER surface ("browser_control NEVER opens the user's real browsers… NEVER narrate a browser_control action as something that happened on the user's computer"), so "I opened Edge" narration of an embedded-panel navigation is now instruction-level forbidden (the receipts discipline + honest-reporting contract from R61/R64 are the behavioral backstops). (2) **THE BROWSER TAB AUTO-OPENS** when the agent browses (the owner: after approving a browser action "the browser never even opened"): every browser_control tool-call + browser-command bridge frame bumps a burst-gated, PROJECT-SCOPED signal (startStream records the session's project); the RightSidebar's edge-triggered controller opens or SURFACES the Browser tab once per burst — never a duplicate blank tab (a sub-agent review caught that + two more real bugs, all fixed pre-commit). (3) **DEBUG MODE** (Settings → Advanced): ON = every agent answer ends with a raw execution report (each tool call, outcome, verification) — the setting is read per turn (next-message effect, like the permission mode); the new GET/PUT /settings/debug follows the memory-switch pattern. (4) **THE ADVANCED CLEANUP**: the agent-core connection / bearer token card is REMOVED (the desktop app manages the engine itself) — Advanced is exactly Debug mode + agent memory; google.com/bing.com joined the default web allowlist (simple browsing asks nothing). Verification: root 1686/1686 in 110 files (was 1664/109), agent-core 894/894 in 53 files (was 885/52), lint/typecheck/docs:check clean, the golden fixture regenerated additions-only, sub-agent review 3-real-bugs-found-all-fixed. Version 0.65.0, TAGGED + RELEASED (the R63 discipline). Full detail in `docs/ui-iterations/round-65.md` · **What's next:** the owner's next live Windows run verifies the R64 backend + R65 boundary together (the runbook checklist), then the standing queue — packaging/code-signing (SmartScreen), ratings-driven prompt tuning, multi-agent support · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
