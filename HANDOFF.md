...
**Last updated:** 2026-08-28 (round-46 DELIVERED — "agent intelligence depth + capability completion": **context compaction** (`agent-core/src/agents/compaction.ts` — over-budget history is SUMMARIZED BY THE MODEL into a dense briefing instead of silently hard-dropped; persists as a `context.compact` session event {summary, throughSeq, droppedMessages, tokensSaved}; pure seq-annotated assembly so fork/revert inherit compactions for free; the newest compaction is reused — no re-summarization per outer-loop iteration; round-2 folds the old summary; summarizer failure degrades to the legacy hard trim; wired into BOTH outer loops + a `meta.compaction` SSE event on the streamed path; 14 tests); **memory v2** (relevance-ranked recall — token overlap ×2 + substring bonus + kind boost + importance×recency tie-break, zero-match rows dropped; ranked digest decision 1.0 > fact 0.8 > preference 0.7 > note 0.5 × recency decay; dedup-on-save bumps updated_at and the tool says 'refreshed'; live-verified through real model turns); **provider-call timeout** (live-battery find: a stalled provider connection hung a turn stuck 8+ min at status 'running' — both chat adapters now wire `AbortSignal.timeout(10 min)`, streamed combines with the caller's signal via `AbortSignal.any`); **checkpoint restore UI** (the round-25 `POST /checkpoints/:id/restore` backend was ORPHANED — no api fn, no button; DiffDetail now has a two-step-confirm Restore pill + toast + query invalidation, hidden for creates because the unlink branch would DELETE the file; plus the pre-existing loadDiff race fix — the first diff card used to bake 'no snapshot recorded' forever; 9 + 2 tests; live-verified: the file on disk actually reverted); **terminal-session e2e** (NEW `tests/e2e/terminal-sessions.e2e.test.mjs`, 4 black-box tests, e2e suite 8→12: create/list/resize/SSE-marker/exit-frame/404s + DELETE-kill); **browser cookie persistence** (per-project SQLite cookie jars — migration 0017, RFC 6265-lite parsing, ingest + replay per redirect hop, session cookies durable by design, values never logged/routed/tool-output, client Cookie headers can't smuggle — test-proven; 16 NEW + browser-proxy 26→33 tests; frontend projectId wire-up deferred, all tabs share the `_default` profile). Version **0.46.0** across all four manifests + CHANGELOG. **622 tests green** (`pnpm test`, 53 files = 610 unit + 12 e2e; was 565 in 49) — full detail in `docs/ui-iterations/round-46.md` · **CI green: run 33219573774 @ c4120e2 (the tip, push event), verified via the Actions API** · **What's next:** the owner's verdict on R46; the bundled-installer workstream (sidecar.rs production spawn — ADR-0003/CHANGELOG `[Unreleased]`); the 1-line frontend projectId wire-up that turns on per-project cookie isolation; memory digest budget tuning; compaction live validation across a real long session · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
