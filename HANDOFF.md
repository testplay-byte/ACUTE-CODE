...
**Last updated:** 2026-08-31 (round-55 DELIVERED — the engine-boot round; every fix traces to the owner's eighth Windows test session, the FIRST one whose log carried the engine's own crash text): **THE EISDIR VERBATIM-PATH CRASH** (the owner's 0.54.0 log showed `spawning \\?\C:\…\node.exe \\?\C:\…\main.js` then `Error: EISDIR: illegal operation on a directory, lstat 'C:'` — Tauri's `resource_dir()` on Windows returns `\\?`-VERBATIM paths and the shell passed them to the child as the program, the script argument, AND the cwd; node starts from a verbatim program path but its module resolver (`fs.realpathSync`/`resolveMainPath`) cannot handle a verbatim entry script — it degenerates to `lstat 'C:'` and dies before the first line of agent-core code, which is why every packaged session since the first build showed "Can't reach agent-core" while dev mode never saw it; `simplified_path()` now strips the prefix off every path that reaches a child process, pinned by unit tests against the owner's exact log paths); **THE CREDENTIAL-MANAGER NAMESPACE SPLIT** (the same log showed `no provider keys found in Credential Manager` one second after the launcher stored all four keys — the keyring crate derives Windows TargetNames as `{user}.{service}` (`api-key.ACUTE-CODE/provider/<id>`) while the launcher's cmdkey stores `ACUTE-CODE/provider/<id>`: two disjoint namespaces since the first packaged build; the keyring crate is REMOVED, replaced by direct CredReadW/CredWriteW FFI in the NEW `src/wincred.rs` at the launcher's canonical targets, with the pre-R55 keyring-form targets read as a legacy fallback and retired on the next Settings save — the spawn now injects all four keys and sidecar.log says so); **LAUNCHER COMMUNICATION** (after an update the ACUTE.bat console prints the "What's new in <version>" changelog panel, and the post-launch guidance explains starting next time: desktop shortcut or ACUTE.bat = update + start). Verification: cargo check GREEN on x86_64-pc-windows-gnu incl. --all-targets + new Rust unit tests, JS gates 1071/1071, live dev-stack battery (real-key turn answered exactly), launcher logic verified in-sandbox. Version **0.55.0**. Full detail in `docs/ui-iterations/round-55.md` · **What's next:** the owner's Windows re-test (double-click ACUTE.bat → auto-upgrade to 0.55.0 → the console should print `✓ agent-core is up — sidecar listening on port N` for the first time), installer code-signing (SmartScreen), the deepseek-harness future candidates, Files-tab polish · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
