...
**Last updated:** 2026-08-29 (round-49 DELIVERED — the file-tools repair round; every fix traces to the owner's second Windows test session after R48): **migration 0019 repairs the default agent's allowlist** (root cause of "I have no write_file/list_dir" on BOTH the main agent and sub-agents: migrations 0014+0015 json_insert-appended the new tools onto the seeded `[]` (= ALL tools per ADR-0019), turning it into an explicit 5-tool allowlist on every pre-R43 database — fresh installs were never damaged because openDatabase runs migrations BEFORE the agent seed, which is why five rounds of green tests never saw it; the repair is fingerprint-scoped, idempotent, audit-logged); **the embedded browser renders full pages** (rewritten sub-resource URLs are now ABSOLUTE against the sidecar origin derived from the request Host — the injected `<base href=upstream>` used to hijack every path-relative CSS/JS/img rewrite onto the upstream origin where it 404'd → blank unstyled HTML; sub-resource errors now return empty bodies with the upstream status instead of HTML error pages parsed as CSS/JS); **nested sub-agents** (children keep delegate_task below MAX_DELEGATION_DEPTH=3 — same tools, own context + key; stripped at the cap as the recursion guard); **the tool-intent nudge** (both turn paths: a zero-tool reply that NAMES a tool or announces delegation gets ONE in-memory correction — "ACTUALLY CALLING the tool(s)" — because free models intermittently write tool calls as text fences; conversational replies still break immediately, the nudge never persists); **the memory master switch** (GET/PUT /settings/memory {enabled}; OFF = no digest injection, no memory tools, amber OFF notice in the Memory panel, data preserved) **+ sub-agent digest isolation** (children never receive the project memory digest — stale memories were actively teaching agents wrong tool sets); **the cat-face logo** (white silhouette — ears, almond eyes, nose as evenodd punch-outs — across sidebar/favicon/icon.ico; VLM-verified "recognizable cat face, no glitches") and **Sessions fully removed** (route + screen + orphaned hook; project sessions untouched) — plus a **live-browser find: the CORS allow-methods list was missing PUT** (every cross-origin PUT — settings toggles, key-pool slots — died at preflight with "Failed to fetch" since the CORS wall shipped; fixed + pinned by a preflight regression test + verified live: the memory switch flips server-side and in the UI). Version **0.49.0** across all four manifests + CHANGELOG. **750 tests green** (`pnpm test` = 62 files; was 752/61 — −13 SessionsScreen + 11 new) + **a 5-check live battery against real OpenRouter keys** (the owner's exact delegation scenario: main → delegate_task → child runs todo_write/create_dir/write_file/read_file/run_command/search_code → ShortStories/story.md on disk with proper markdown; memory settings round-trip; migration on the real dev DB; browser proxy absolute rewrites + stylesheet round-trip) — full detail in `docs/ui-iterations/round-49.md` · **What's next:** the owner's re-test (especially sub-agent delegation + the embedded browser rendering on his machine, and whether his default free model needs swapping for reliable structured tool calls); the chat bottom-bar/model-switcher round (owner explicitly deferred it — "let's talk about it later"); the bundled-installer workstream (ADR-0003/CHANGELOG `[Unreleased]`); a slot-aware `store_provider_key` in the Tauri shell; the 1-line frontend projectId wire-up for per-project cookie isolation · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
