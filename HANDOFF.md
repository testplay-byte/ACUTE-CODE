...
**Last updated:** 2026-08-29 (round-47 DELIVERED — "provider management, clean + reliable" + the launcher key scrub): **one provider API layer** (`src/lib/api.ts` is now the ONLY provider plumbing — fetchProviders/createProvider/updateProvider/deleteProvider/storeProviderKey/testProviderConnection(id, {model?, slot?})/fetchModelsCatalog + the models-config CRUD quartet, all typed against the real agent-core wire shapes; ModelsProvidersTab's local useApi() — the third parallel HTTP layer — DELETED; dead legacy ApiTab/ProviderKeyCard deleted); **slot + model-scoped connection tests** (POST /providers/:id/test accepts {model?, slot?} — a probe can now prove a specific key-pool slot AND a specific model; empty slot → honest 409, invalid → 400; ok:false at HTTP 200 resolves; the UI shows server-measured latency in green / refusal messages in red, live-verified "Connected · 1207ms · z-ai/glm-5.3-flash"); **the served model catalog** (NEW GET /models/catalog — 46 models, 18 free, defaults + recommended — replaces every hand-copied duplicate: SubAgentsTab's 47-entry copy deleted, AgentFormDialog's hardcoded provider list + dead openrouter/ox-alpha placeholder gone, catalog-fed datalists added); **enabled enforced at turn time** (prepareTurn → 409 PROVIDER_DISABLED, disablement wins over a missing key, session stays retryable — the Enabled toggle was cosmetic since R37); **raw-key route removed** (GET /providers/:id/key returned the RAW key — zero callers, now 404); **launcher key scrub** (the R44 leak: real KEY_2/3/4 were committed in credentials.example.txt AND baked into acute_launcher.py's DEFAULT_SUB_KEYS — both gone; template redesigned as a paste zone with placeholders; the launcher never writes key values; PLUS the R44 parser bug this surfaced: names containing digits never parsed, so the owner's own sub-key values were silently ignored since R44 — fixed); **KeyPoolSection slot-collision fix** (slots.length+2 overwrote slot 4 when 2 & 4 were held; now a pure nextFreeSlot helper, unit-tested). Security note: the scrubbed keys remain in git HISTORY (private repo) — the owner should rotate KEY_2/3/4 at OpenRouter when convenient. Version **0.47.0** across all four manifests + CHANGELOG. **683 tests green** (`pnpm test`, 56 files = 671 unit + 12 e2e; the R46 record was 622 in 53) — full detail in `docs/ui-iterations/round-47.md` · **CI green: run 33243501373 @ 0643ac9 (the tip, push event), verified via the Actions API** · **What's next:** the owner's verdict on R47; the bundled-installer workstream (sidecar.rs production spawn — ADR-0003/CHANGELOG `[Unreleased]`); a slot-aware `store_provider_key` in the Tauri shell (pool keys pasted in the desktop app currently land on the primary key — src-tauri was out of scope this round); the 1-line frontend projectId wire-up that turns on per-project cookie isolation; compaction live validation across a real long session · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
