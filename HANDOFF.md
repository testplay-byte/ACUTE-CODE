...
**Last updated:** 2026-09-02 (round-62 DELIVERED — the owner-feedback round): (1) the COMPUTER-USE PRESENCE report was verified LIVE against the repo tip — the Settings → Computer Use section, master switch, vision config, readiness probe, monitor, and mini window are ALL present (the report matched a stale local build; pull 0.62.0); (2) the SIDEBAR MINIMIZES now — a 64px icon rail (restore at the very top, nav icons, project tiles with live dots, bell, gear), persisted, orthogonal to the title-bar full-hide; (3) the SESSION SCREEN's gradient background layer is REMOVED (flat theme background; padding + rounded corners kept, gap tightened); (4) the EMBEDDED BROWSER RESPECTS THE SET DIMENSIONS — computeNativeBounds is now an aspect-fitter that composes the fit into the DPI zoom so the PAGE sees the full preset CSS px (the "fits N%" readout; the clamping lie is gone) + the Chromium footnote strip is REMOVED; (5) MENUS NO LONGER RENDER UNDER THE WEBVIEW — a DOM overlay watcher (menu/dialog/popper/[data-overlay], tooltips excluded) hides native webviews while any overlay is open (the R60 popover guard is store-backed and preserved; a stray-quote selector boot crash was found live + pinned); (6) THE AGENT USES THE RIGHT-SIDEBAR BROWSER: browser_control grew read (page text, server-side), eval (JS in the LIVE page via the SSE→UI→Rust browser-command bridge + the new browser_tab_eval command — native desktop mode), screenshot (the panel's physical-px region via the computer-use relay + the vision pipeline), and get_state now lists every tab; the prompt section teaches the triad (golden regenerated); (7) SETTINGS → APPEARANCE simplified to Theme/Chat density/Tool activity (Sidebar Tint removed); (8) MODELS & PROVIDERS reflect on the session page immediately (cross-cache invalidation fan-out), per-1M prices clearly labeled with decimal support, a Supports-vision toggle in the dialog, config-only models appear in the picker, disabled providers filtered, and computeCost's either-null → $0 gate fixed (partially-priced models cost their known sides). Verification: root **1542/1542 in 104 files** (agent-core **801/801**), lint/typecheck CLEAN, LIVE battery green incl. a REAL model turn driving browser_control get_state/read/eval. **HONEST CAVEATS: the agent-browser's NATIVE paths (eval in the page, screenshot+vision) are code-complete + unit-tested but live-unverified in this headless sandbox — verify on the desktop build (ask the agent to open a page, read, eval document.title, screenshot with Computer Use ON); the computer-use platform backends remain live-unverified per the R61 checklist; macOS region capture is point-space (Retina may need scale division).** Version 0.62.0. Full detail in `docs/ui-iterations/round-62.md` · **What's next:** the live-verification checklist (computer-use + agent-browser native paths on the real desktop), then the standing queue — multi-agent support ("after this we will work on the improvements functionality and management": the smarter main agent with flex options, helper agents, agent self-browsing/testing of built web apps, automation tools, everything module/plug-in — the ratings loop feeding prompt tuning), packaging/code-signing · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
