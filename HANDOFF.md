...
**Last updated:** 2026-09-14 (round-97 DELIVERED: the eighth-walkthrough round, v0.94.0 → v0.95.0 — the owner's v0.94.0 report answered end to end: THE CONTEXT POPUP REBUILT (the Kilo-style SEGMENTED BAR at the top of the usage card — one segment per category, compact counts flanking, the dimmed reserved block — plus the donut + the SESSION COST headline, the breakdown rows with palette dots + mini-bars, the cache hit-rate, and the session TABLE Main/Sub-agents/Combined × Turns/Calls/Sent/Received/Cost, with Cursor's MUTUAL hover-highlight; the DOM popover renders the same bar — the views are twins, nothing hidden), THE THINKING LOOP SET FREE (a Settings domain, master switch DEFAULT OFF per the owner's directive "so that the model can think as much as it needs to" — enabled arms the user's OWN stall-seconds/reasoning-KB thresholds; inputs commit on blur/Enter; the stop renders the AMBER "Thinking stopped by the guard" card — never red, never "Generation failed"), THE FAILURE-USAGE TRUTH (a failed turn reports the tokens it actually spent: the chip + Copy details carry the completed iterations + EVERY retried attempt's streamed-so-far — a ladder that burned 50K on attempt 1 no longer reports the instant final death; live card AND folded; never a fabricated 0/0), REAL SYNTAX COLORS (prismjs, 27 grammars, the app's own light+dark palette — the thinking area's fenced code included; the line-number gutter PRESERVED via the token-stream line split), THE CHAT'S CUSTOMIZABILITY (Appearance → Text Size S/M/L scaling the answer+thinking+narration together, capped ±12%; Timestamps On-hover — the time chip joins the user bubble's hover cluster + the turn footer's meta row), THE STATE-AWARENESS SWEEP (the chat's FALSE GREETING dead — a chat-shaped skeleton while the log loads, the retryable "Could not load this conversation" card on failure, a populated transcript always wins; the sidebar's false "Add your first project" dead; the dashboard never paints false zeros; the five settings cards' eternal "loading…" dead — honest error + Retry; shared Skeleton primitives + the amber warning token in the design system), THE BROWSER'S SETTINGS HOME (its own Settings section: search engine, homepage, default zoom with the store-persisted zoomTouched sentinel, 1-12 editable quick links — all wired; the HOME button made to WORK: homeView + the poll-anchor against the re-adopt, the native webview hidden while home — the review's only must-fix), and THE REVIEW PASS (a dedicated audit of the whole round's diff: zero blockers, every finding fixed + pinned — the HOME no-op, the usage contract's runtime+frontend halves, the code gutter's silent drop, the commit-on-blur steppers, the ladder burn, and eight more). Full pipeline: lint, both typechecks, root 3,498/3,498, agent-core 2,255/2,255, e2e 12/12, docs:check 204/0/0, version ×4 at 0.95.0. The owner's TEST CHECKLIST lives in round-97.md §9.) The prior state: the seventh-walkthrough round, v0.93.0 → v0.94.0.
