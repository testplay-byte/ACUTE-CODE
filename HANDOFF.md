...
**Last updated:** 2026-09-08 (round-77 DELIVERED — THE FIELD-REPORT ROUND, the owner's 0.75.0 report, seven verbatim asks, all implemented + live-verified): (1) the composer toolbar split LEFT (attach + access + task mode) / RIGHT (context donut + model + reasoning + send) with the right cluster a single nowrap `ml-auto` group — the send/stop action can never wrap below alone (the owner's ask-3); (2) the model pill leads with a Cpu icon (every toolbar control icon-led now; icon-only below the 560px container floor); (3) the RetryStatusCard redesigned — circular spinner chip, six-dot attempt ladder (failed dim, upcoming breathing via the new ac-retry-pulse keyframes, future hollow), a countdown fill-bar driven by waitMs, rounded class pill; (4) THE VANISH FIX (the root cause behind "tried sending a message, but it was not that successful"): a turn that failed WITHOUT a persisted turn.error — pre-hijack rejections (validation/auth/409/PROVIDER_DISABLED) and dropped streams — used to wipe BOTH the live error card AND the optimistic user bubble (clearStream deleted the whole slice), leaving the transcript looking like the send never happened; NEW stream-store `freezeFailedTurn` keeps the error card + the user's message visible until the next send, the api.ts non-2xx branch now preserves the envelope's REAL code + details (no more PROVIDER_ERROR masquerade), TurnErrorCard grows a Show-full-error toggle (>240 chars → the complete raw provider text in a scrollable mono block), and runTurn's catch always surfaces the real ApiError text; (5) revert-to-message is now edit-and-resend (owner spec verbatim): the backend truncates FROM the target message INCLUSIVE (DELETE seq >= keepThroughSeq) and the panel pastes the message's text back into the composer, focused — the dialog + toast say exactly that; (6) the live E2E battery (real OpenRouter, varying difficulty): smoke, 3-file static site, python todo CLI with self-run green tests, 3-turn build→extend→refactor with tests green at every stage, a 10-turn conversation (codename + tech-stack edits recalled 6-7 turns later), revert over the wire, error surfacing — 20 turns, 46+ tool calls, avg ~18 s/turn, $0.00; (7) THE BLANK-OUTPUT GUARD (born from the battery's T5 turn-10 flake — glm-5.2:free answered with whitespace, the turn "completed" ok, the requested edit never happened): a fully-blank zero-tool turn now ends with an honest NO_OUTPUT 502 + a persisted retryable turn.error. Two genuine free-tier 429 storms during the battery live-verified the R75 ladder end-to-end (frames at 90 s/5 m/10 m/30 m, ~49.6-min full-ladder exhaustion, the amber card redesign rendering live, the honest exhaustion error carrying the REAL "Rate limit exceeded: free-models-per-day" text, the boot sweep's INTERRUPTED event after a mid-ladder kill). Verification: root 2627/2627 (925 src + 1690 agent-core + 12 e2e; +14 new tests), lint clean, both typechecks clean, version 0.76.0 ×4, docs:check 177/0/0; browser-verified end-to-end (cluster DOM, Cpu icon, revert flow, Stop, retry card, error card — 4 screenshots). **R76 CLOSED BEFORE THIS ROUND: docs-only (acbc5a4 → CI 34159808321 first-push green, docs:check 176/0/0, no release — the app byte-identical to v0.75.0).** The R77 release close-out (tag v0.76.0 → Release → publish) follows this commit per MAINTENANCE recipe g. · **What's next:** the R77 queue: delegate_task task_id/background/resume (the standing R73 deferral), external plugin ctx enrichment (cline's appendContext seam), lessons-ledger as the reminder injector's third consumer, ratings-driven prompt tuning, the optional v0.68.0 backfill · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
...
## 3. Exact current state (2026-09-08, round-77 DELIVERED — v0.76.0)

**R77 is DELIVERED: the 0.75.0 field report's seven asks shipped, unit-tested (2627 + 1690 + 12), browser-verified, and live-battery-verified against the real provider — see `docs/ui-iterations/round-77.md`.** R75 is CLOSED (v0.75.0 published: release 384282923, commit 66e949a, CI 34154269987 first-push green); R76 is CLOSED (docs-only, acbc5a4 → CI 34159808321). The R77 release close-out (tag v0.76.0 → Release → publish) follows the R77 commit per MAINTENANCE recipe g. The round-28-era table below is kept as the historical record.
