...
**Last updated:** 2026-08-24 (round-33 DELIVERED — THE hello-loop fix: zero-tool iterations break the outer loop, live-proven ("hello, how are you" = one clean reply, no rounds); activity interleaved at the point tools ran (no ROUND labels, stats on the final message only); live file-population view; the full sidebar system (AcuteLogo with hover-morph toggle, collapse top-right, section spacing, on-project + session button, renameable sessions via PATCH /sessions/:id, prominent Settings card, name-from-folder, instant list updates); chat panel fully headerless with Ctrl K labels on Windows + floating-logo show-sidebar. **215 tests green** + live OpenRouter battery + VLM. Design prompts 2 (composer) + 3 (settings) await the owner's demos) · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
