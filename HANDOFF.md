...
**Last updated:** 2026-09-07 (round-74 DELIVERED — THE UPDATER-FREEZE ROUND, the owner's field report "the installed desktop app version was 0.67.0 — the application apparently did not update" answered root-cause to remedy; the round's code + docs are complete — the close-out (commit + CI + tag v0.74.0 + release + DASHBOARD truth-sync) is the orchestrator's next step): (1) **THE ROOT CAUSE** (proven against the live GitHub API): the release list sorts never-published DRAFTS above every published release — the R63–R67 close-outs left five drafts (v0.63.0–v0.67.0) at list positions 1–5 — and _desktop_latest_release's first-match walk over that order returned draft 0.67.0 as "latest", so the owner's installed 0.67.0 looked current while 0.73.0 was live: no error, nothing to see, the update simply never triggered (and it "used to work" because in the 0.63–0.67 era ALL releases were drafts, so first-match landed on the newest anyway). (2) **THE FIX** (launcher/acute_launcher.py, the first launcher change since R63 — flagged per golden rule 1; ACUTE.bat BYTE-IDENTICAL, CRLF verified): NEW pure _pick_latest_release(releases) — walks EVERY entry, parses versions from matching installer assets, picks the numerically GREATEST (_version_tuple integer tuples, so 0.10.0 > 0.9.0 and 1.0.0 > 0.99.99); drafts stay first-class (the owner's PAT sees them — a fresh draft installs on the next double-click, the recipe-g contract); a same-version tie prefers the published entry; returns (version, asset_id, digest, info{tag,draft,created}); malformed entries skipped, never raised (strictly more defensive than the old walk, which raised TypeError on a null asset name). _desktop_latest_release rewritten onto it: per_page=20→100, ONE retry on transient errors (the download step's pattern), never-raise + dev-flow-fallback semantics byte-identical. Both call sites updated; the version-truth panel's release line now NAMES the tag it picked ("0.74.0  [v0.74.0]" / "[draft v0.75.0]") and ACUTE.bat status annotates published/draft — the freeze is VISIBLE, not silent. (3) **THE REGRESSION TESTS** (NEW launcher/tests/test_pick_latest_release.py — the repo's first Python suite, stdlib unittest, zero deps, loads the real module importlib-safe): 10 tests — the exact owner-freeze list shape (five drafts above the published releases → 0.73.0, with the chosen asset's id + digest passthrough — the sha256 install verification depends on it), order immunity both directions, the lexicographic trap, major-beats-minor, the all-drafts era, tie-prefers-published in both list orders, no-assets→None, empty→None, malformed-never-crash, non-matching asset names — WIRED into ci.yml's verify job (python) AND release.yml's launcher-kit gate (python3; github-release needs the kit job → every tag gated); the kit assembly copies seven named files only, tests/ cannot leak into the shipped zip. (4) **THE REMEDIATION** (out-of-band, GitHub API): the five stale drafts published (releases 381461843/381577121/382282125/383173421/383247624, all HTTP 200 — the R69+ close-out step those rounds never got; non-destructive, reversible) — which also un-freezes any install still running the pre-fix launcher (the settled list now carries v0.73.0 first, so the OLD walk returns "upgrading 0.67.0 → 0.73.0"); the R74-b hostile verifier simulated the old launcher against every list state it could see and proved the never-downgrade guard holds in all three. The fix reaches the owner's launcher via its own SELF-UPDATE: next double-click → git pull → self_update_check sees the kit's 0.63-era file differ → copy + os.execve re-exec → the fixed picker runs → "upgrading 0.67.0 → 0.74.0" → sha256-verified download → silent install → registry + exe + engine verified → launch. Anomaly flagged per the standing rule: v0.68.0 NEVER EXISTED (R68 shipped the version bump but skipped the tag push — the exact R61/R62 failure class; left as a documented gap, superseded by 0.69.0+; a v0.68.0 tag on the R68 commit would backfill it in one workflow run if the owner wants complete history). Verification: root vitest 2558 UNCHANGED and re-run green (2,546 passed + 12 e2e env-gated, 138 files); the NEW launcher suite 10/10; py_compile clean; lint + typecheck exit 0; the LIVE smoke — the real /releases JSON through the picker → 0.73.0 (the exact data that froze the owner) — run by the orchestrator AND independently by the R74-b verifier (9/9 PASS, verdict SHIP). Full detail in docs/ui-iterations/round-74.md · **What's next:** (a) the orchestrator's close-out (commit + CI + tag v0.74.0 + release + DASHBOARD truth-sync — milestone 55 + feature card 116 live in the DASHBOARD's data.json, NOT status.json), then (b) the owner's live gates (the panel naming the tag, the 0.67.0 → 0.74.0 upgrade end-to-end, status's published/draft line) PLUS the R69–R73 gates still pending, then (c) the R75 queue: delegate_task task_id/background/resume (the deliberate R73 deferral), external plugin ctx enrichment (cline's appendContext seam), the lessons-ledger affordance as the reminder injector's third consumer, ratings-driven prompt tuning, the optional v0.68.0 backfill · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
