...
**Last updated:** 2026-09-14 (round-96 DELIVERED: the seventh-walkthrough round, v0.93.0 → v0.94.0 — the owner's v0.93.0 report answered end to end: THE SMARTER AGENT (precision tools — whole-file-first reads, ripgrep-style search, atomic multi-edit + variant rungs; the BATCH/COMPLETION/PRECISION prompt disciplines; four new seeded skills + the search_skills discovery tool; the skills listing budget-capped), THE LOOP GUARD REWRITE (WARN-ONLY per the owner's directive, RAW-args identity — the paged-read false positive is dead in both turn paths; identical-text detection; warnings never fail a passing turn), COMPLETION = THE MODEL'S OWN STOP (the "The last message must have role=user" failure is structurally impossible now — the messages-shape guarantee; a failed post-completion continuation keeps the completed answer), THE STUCK STOP BUTTON (terminal frames retire the busy state at the frame + an 8s never-settling-stream watchdog), THE REASONING LADDERS VERBATIM (xhigh/max are real rungs; the wire sends the model's own token — effort "max" on the owner's exact model, live-verified; the menu names its source; legacy models auto-refresh), THE BROWSER (the usage hover rides the overlay window — no more "Browser paused while the menu is open"; agent-navigate == address-bar parity incl. file:// re-reads; a loading edge), THE CHAT WINDOW (collapsible red/green unified-diff cards on edit/write rows; richer tool lines; the donut's value hover-only), THE INSTALLER KILL-ORDER (the sidecar tree dies before the installer launches — the win32 x64.node/node.exe write race), and THE LIVE-FIRE (a 258-file synthetic project against REAL OpenRouter: the precision task + the paged-read silence + the wire shapes + token truth ALL verified; TWO real wire bugs caught + fixed — the OpenRouter app-attribution headers and the max_tokens output cap). Full pipeline: lint, both typechecks, 2,228 + 1,193 tests, docs:check. The owner's TEST CHECKLIST lives in round-96.md §9.) The prior state: the sixth-walkthrough round, v0.92.0 → v0.93.0.
