...
**Last updated:** 2026-09-02 (round-60 DELIVERED — the second owner-feedback polish round; the thirteenth Windows test session confirmed the rounded window PERFECT — "keep it as a part of our design language" — and the pop-out browser satisfying; its polish list became this round, ALL delivered): (1) the pop-out VIEW is ROUNDED (the owner: "the thing which was not rounded off was the actual view") — the content sits in a rounded-[14px] bordered card matching the title-bar/URL-bar design language, the webview placeholder inset 4px so the native square's corners stay inside the card's curve; (2) the pop-out GUTTER SCROLLBAR (the owner: "The scroll bar should be custom themed on every single page. It should not show inside the section but on the right side outside it") — the window paints its OWN scrollbar in the frame's right gutter, OUTSIDE the content card: a floating pill (drag/click/keyboard, 250ms eval_with_callback polling, rAF-throttled scroll_to) driven by three NEW Rust commands (browser_tab_scroll_state / browser_tab_scroll_to / browser_tab_set_zoom — the scroll channel reads the page's scroller geometry OUT of the external webview, doc-verified against tauri 2.11.5; set_zoom is REAL DPI zoom); pages get themed-pill scrollbars via a document-start initialization_script (browser_tab_create's optional hide_viewport_scrollbar hides the pop-out page's viewport bar; CSP-strict pages keep their own — never two bars); (3) the API-KEY FIELD is one consistent row (the owner: "the show and hide button should be in the exact same place. The copy button should appear smoothly… no need to give the rotate key option") — same-slot eye toggle (DOM-identity-pinned in tests), framer-motion Copy fade, Rotate DELETED, paste-a-new-key + Save/Cancel (Escape restores); (4) MODELS start EMPTY (the owner: "By default none of the models should be added there") — the list shows only stored rows (the catalog merge is gone), the Add-models picker carries the "Free only ↔ All models" toggle (shared persisted pref), every added model (incl. free) fully configurable; (5) SETTINGS PADDING stripped (the api tab fills the width) + the SCROLL-REFLOW width-shift FIXED (.auto-scroll: scrollbar-width thin + scrollbar-gutter stable always — the gutter is permanently reserved); (6) the TITLE-BAR SIDEBAR TOGGLE (the owner: "I don't want the app's logo on the sidebar… I would need to click the app's logo at the top left corner") — the sidebar's logo + collapse button + the 64px rail are DELETED; the TitleBar identity block (logo + name) toggles the global appSidebarVisible (sidebar present on every route including chat until hidden; web mode keeps the floating-logo fallback); (7) the BROWSER-PANEL FIXES — the new-tab popover no longer hides BEHIND the native webview (the active browser webview yields to the popover via popover-webview-guard, covering the create-while-suppressed race), the zoom select ACTUALLY zooms (real DPI zoom; the R50 viewport-divide approximation deleted), the viewport bar is a two-group rounded card (fit honestly disabled in native mode), bounds re-sync immediately on viewport changes; (8) the RATING POLISH — re-rating a saved bad reply pre-fills the note editor; a "noted" chip surfaces saved notes. Verification: root suite **1348/1348 in 92 files** (was 1302/88), lint/typecheck CLEAN, LIVE agent-browser battery on the dev stack (empty models default, picker free-only default → All = 300 rows, add/delete model round-trip, key reveal/hide/paste-cancel with zero PUTs, computed {thin, stable, transparent} scroll styles, 974px-of-1280 settings width, sidebar toggle on dashboard + chat routes, popout.html 200, zero console errors); Rust doc-verified against tauri 2.11.5 with CI cargo check as the compile gate. Version **0.60.0**. Full detail in `docs/ui-iterations/round-60.md` · **What's next:** the owner's Windows re-test of 0.60.0 (pop-out rounded view + gutter scrollbar on real sites incl. CSP-strict ones, key paste-to-replace, empty models + picker toggle, title-bar sidebar toggle, new-tab menu over the browser preview, real zoom), then the owner's stated future directions ("after this we will work on the improvements functionality and management": the smarter main agent with flex options, multi-agent support with helper agents, agent self-browsing/testing of built web apps, automation tools, everything module/plug-in — the ratings loop feeding prompt tuning), installer code-signing · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
