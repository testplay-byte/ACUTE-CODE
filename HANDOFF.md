...
**Last updated:** 2026-09-13 (round-95 DELIVERED: the sixth-walkthrough round, v0.92.0 → v0.93.0 — the owner's v0.92.0 report answered end to end: THE MODELS & PROVIDERS UI OVERHAUL (the taller fixed provider rail; sidebar-only back navigation; Key 2..N parity with Key 1 with styled ConfirmDialogs replacing every browser confirm; the add-then-configure picker — pencil gone, already-added models hidden; hidden-at-bottom LIVE animated model sorting with scroll persistence; the one-Test-button three-option single-row scope picker; provider delete at the TOP with the dedicated confirmation), THE PROVIDER-TEST TRUTH (the reachability probe carries the provider's raw upstream error body — the actual error message, always), MODEL-AWARE THINKING (migration 0035: per-model reasoning capability from OpenRouter's supported_parameters — verified against the live catalog; the thinking menu adapts to each model's ladder; medium joins the vocabulary; the wire mapping never sends an unsupported effort; reasoning budgets bound ladder-less models — and effort XOR max_tokens on the wire, the mutual exclusion OpenRouter enforces that the live-fire CAUGHT), THE THINKING-LOOP GUARD (>120s of pure reasoning with >24KB accumulated → ThinkingLoopError → ONE de-escalating retry with the visible meta.retry card, then honest failure — "stuck in thinking" no longer hangs), THE NATIVE BROWSER OPENS LOCAL FILES (file:// end-to-end: Rust parse_web_url, the panel's local-path normalization for the address bar + agent navigations, the sidecar's ticket-gated local-file route, the agent tool's bare-path navigate + disk-backed read — the owner's "proper native browser" directive), THE CHAT (the thinking area stick-to-bottom + inner jump pill; the nested-scroll chaining fix — the chat pill no longer appears while reading the thinking tail; three real markdown bugs fixed with regression pins — peeled punctuation vanishing from autolinks, trailing emphasis glomming onto URLs, quoted file paths losing their pills; the context donut's measured-usage prominence; the entry-renderer registry — the modular foundation for future info types), and THE REAL-API LIVE-FIRE (scripts/r95-live-fire.mjs: the built sidecar through a recording proxy against REAL OpenRouter with the free Nemotron models — the SKILLS section verified in the live system prompt, the effort mapping verified on the wire, and the TOKEN USAGE TRUTH: the app's numbers EXACTLY match the provider's, zero deviation). Full pipeline: lint, both typechecks, 3,258/3,258 (175 files), e2e 12/12, build, license 134. The owner's TEST CHECKLIST lives in round-95.md §9.) The prior state: the field-report round, v0.91.0 → v0.92.0.
