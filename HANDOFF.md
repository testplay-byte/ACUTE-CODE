...
**Last updated:** 2026-09-09 (round-81 DELIVERED, v0.80.0: THE UNIFIED OPERATING MODES — the owner's one-picker directive shipped as ADR-0029: exactly THREE modes (Full Access = all tools, zero asks, the agent decides how to work and switches postures itself via switch_mode; Ask = default, full tools with gates before important commands; Plan = read-only, now carrying the full 19-tool read-only vocabulary incl. the git inspectors/analyze_image/job_status). Task modes became NON-ENFORCING agent self-selected postures — the six bodies + .acute/agents customs survive with 'Discipline, not permission (R81)' prose, the prompt sections read OPERATING POSTURES/ACTIVE POSTURE/OPERATING MODE; the R75 task-mode policy tier, debug command tier, and switch_mode owner-pin are RETIRED. 'editor' retired fail-closed — migration 0029 maps editor→ask and read-only R75 postures→permission plan (the guarantee preserved); PATCH /permissions rejects editor with an honest 400 hint; GET /modes readOnly is always false. Frontend: TaskModePicker + the /mode slash + the modes query + api.ts's TaskModeInfo/fetchProjectModes/patchSessionActiveMode deleted; the composer's left cluster is attach + THE unified picker (AgentChatPanel 2,932→2,743 lines). Verified: 2,751 root tests (12 skipped) + e2e 12/12 + a LIVE OpenRouter battery — a PLAN turn's write_file attempt CANNOT execute (no file written, the model answers honestly 'no file-writing tool is available'), a FULL turn writes full-proof.txt, an ASK turn gates commands, switch_mode persists a posture (activeMode=explore) — plus browser + VLM verification of the 3-option menu (exactly Full Access/Ask(default)/Plan, no Editor, no task-mode pill, zero console errors). Full detail: docs/ui-iterations/round-81.md · ADR: docs/decisions/0029-unified-operating-modes.md · spec: agent-ctx/research/task-mode-consolidation.md). · **What's next:** the R82+ queue — Wave 2 of the modularity roadmap (server.ts route split, turn-loop harness, the ToolDeps SCC break), models & providers hardening (the custom-provider→OpenRouter misroute fix — providerId must ride the send wire; the per-model test button; capability toggles), honest token/context metering (the donut's estimate-as-fact problem), then Waves 3-4 · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
