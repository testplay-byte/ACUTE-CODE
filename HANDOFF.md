...
**Last updated:** 2026-08-31 (round-56 DELIVERED — the launch-choice round; the owner's ninth Windows test session reported "still not working it faild and i think in the acute.bat it should ask how to launch the app or the site" — no new log attached, so the round ALSO audited every remaining packaged-boot crash candidate before shipping the request): **THE LAUNCH QUESTION (the owner's request, verbatim)** — every interactive run now asks how to launch: [1] the desktop app (packaged window, embedded browser, bundled engine) or [2] the site (local servers + browser at http://localhost:5173); Enter keeps the last choice (remembered in .acute-launch-pref.json next to the launcher, first run defaults to the desktop app); `ACUTE.bat app` / `ACUTE.bat site` (or `--app`/`--site`/`--web`/`--no-desktop`) skip the question; non-Windows is always the site; non-interactive runs use the remembered choice silently; the ask runs AFTER the update pass so the newest launcher code asks it. **ENGINE-FAILURE RECOURSE** — when the desktop engine does not report ready (startup failure, timeout, or the app exiting) the launcher prints the engine's log tail and asks: [1] retry (close + relaunch, one round), [2] use the SITE instead (closes the app, falls through to the browser flow — the exact escape requested), [3] keep the desktop app (offline screen has Restart-engine + Copy diagnostics); a dead engine is never a dead end. **THE SELF-UPDATE RE-EXEC** — the launcher's self-update previously took effect one run LATE (every launcher improvement arrived on the run after the one where it mattered); the fresh copy is now exec'd in place (ACUTE_LAUNCHER_REEXEC loop guard, args + exit code preserved) so new launcher logic drives the session it arrives in. **THE WATCH'S TIMEOUT IS NO LONGER BLIND** — a cold boot that outruns the watch used to print "did not report ready within 45s" with zero diagnostics (the exact silence shape that hid the EISDIR crash for three sessions); the watch now runs 75s and prints the engine's last output on timeout; the launch line names the version; `ACUTE.bat status` reports the launch preference + the engine's last successful boot line. Audit verdict on the 0.55.0 boot path (nothing else found): simplified_path correct, better-sqlite3 13 N-API prebuilds ABI-safe on pinned Node 24.20.0, node-pty loads defensively, CORS allowlists the tauri webview origins, CSP null, staging sound. Verification: all launcher paths logic-tested in-sandbox against the real file (mode resolution, ask parsing + persistence, recourse branches end-to-end through desktop_flow, exec mechanics with a REAL process replacement + loop guard), JS gates 1071/1071, cargo check green on x86_64-pc-windows-gnu, live dev-stack battery with a real-key turn (answered exactly R56-LAUNCH-CHOICE-VERIFIED). Version **0.56.0**. Full detail in `docs/ui-iterations/round-56.md` · **What's next:** the owner's Windows re-test (double-click ACUTE.bat → the self-update re-exec line → the app-or-site question for the first time → either `✓ agent-core is up — port N` or the log tail + retry/site/keep), then the R50 queue (command watchdog polish, sub-agent supervision stats UI, terminal output visibility, usage screen section 2, model-selector hover, plugin tool system), installer code-signing · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
