...
**Last updated:** 2026-09-11 (round-90 DELIVERED: the browser-native round — the owner's second end-to-end walkthrough answered, v0.87.0 → v0.88.0. THE MENU OVERLAY WINDOW (the headline verdict: "the menu was supposed to be shown on top of the browser window itself"): the sidebar's quick menu + sub-agent picker render in an OWNED transparent borderless OS window that rides ABOVE the live browser webview (Win32 owned windows stay above their owner, child webviews included — browser.rs's menu_overlay_prewarm/show/hide/pending + menu-overlay.html, the 4th vite entry + src/lib/menu-overlay.ts, the main window's bridge + src/menu-overlay/, the payload carries kind/title/items/theme; the browser NEVER pauses for these menus; the R60-D webview-hide survives only as the web-mode fallback). THE AGENT HANDS v2 (the owner's browser-behavior verdicts): the cursor is ALWAYS VISIBLE (the tab webviews' INITIALIZATION SCRIPT — browser_tab_create's hands_init_script ← src/lib/agent-hands-boot.ts — paints the resting cursor at document creation on EVERY page; the runtime ADOPTS it via window.__acuteHandsRest), the HUMAN PATH PLANNER (overshoot-and-return ~45% of moves >90px, mid-flight hesitation ~22% of moves >200px, bezier legs + micro-jitter + arrival settle — the owner's "go a little bit further away and then return back"), the REAL EVENT TRAIL (every travel frame fires pointermove/mousemove + hover over/out — hover-driven dropdowns actually OPEN now), the VISIBLE CLICK (a settle micro-move + a PRESS PULSE — the cursor squashes to 0.82 and springs back — + a genuine 60–140ms held duration), and the HUMAN-PACED TYPE (TAP the field → a 0.7–1.3s beat → word-by-word ~150 WPM → submit: a 0.8–1.4s beat → the synthetic Enter sequence + requestSubmit; job budgets raised 25s→60s panel / 30s→75s sidecar). THE SETTINGS CLUSTER: the delete-provider refusal renders INLINE in the Danger zone (the old dead-click surfaced 1500px above in the accent color) with a pre-armed "in use by N agents" warning, and the OS-stored key is PURGED on delete (remove_provider_key — the keys.rs R82 TODO closed); the model card is ICON-ONLY capability badges + an inputs→outputs ARROW + ONE merged highlighted section (identity row + stats band fused); the setup wizard PERSISTS the choice (the model row with context/max-output/pricing via upsertProviderModelConfig, saveLastUsedModel, and the agt_default_nova PATCH — the "model was not selected there" + "the provider was OpenRouter even though I did not have OpenRouter added" verdicts), and the session-switch effect keeps the global last-model fallback. THE LAUNCHER PAT: saved in the USER HOME (~/.acute/github.pat, the kit copy MIGRATED) + passed to the app via ACUTE_GITHUB_PAT — the "Check for updates" message after running acute.bat is gone. THE FLOATING-WEBVIEW RACE: nativeCreate's .then() consults a mountedRef latch — a create resolving after the panel unmounted HIDES instead of re-showing over the new chat. Full pipeline green: frontend 2,888 + agent-core 1,894 tests, build (4 pages), e2e 12/12, license audit clean, 16/16 launcher python tests. The owner's TEST CHECKLIST lives in round-90.md §6.) · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
