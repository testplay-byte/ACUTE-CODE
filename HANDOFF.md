...
**Last updated:** 2026-08-30 (round-50 DELIVERED — the native-browser + composer round; every workstream traces to the owner's third Windows test session after R49 — he CONFIRMED the cat logo, the memory switch, and the R49 tool repair, all frozen): **the native embedded browser** (the R43 fetch-proxy iframe is superseded INSIDE the desktop shell by real Chromium: one WebView2 CHILD WEBVIEW per browser tab, hosted in the main window via Tauri multiwebview — `Window::add_child`, which required `tauri features=["unstable"]` and an ASYNC create command to dodge the documented WebView2 sync-command deadlock; bounds synced from the panel's placeholder rect (ResizeObserver + resize + 500ms net, rAF-debounced) through `computeNativeBounds`; a shared persistent `browser-profile` dir across tabs; `browser-navigated` events keep the sidecar's server-side history truthful for `browser_control`; the iframe proxy survives only as the non-Tauri fallback); **sub-agents stream their raw thinking/text LIVE** (children now run `runStreamedAgentTurn` when a channel exists — `chatStream` flows into toolDeps — so the panel shows real text-delta/thinking-delta frames as they generate, 75+218 live frames in the live battery's delegation round) **+ 5-attempt retries** (the owner's "failed after three attempts" was the AI SDK default maxRetries:2 — now 4 retries on both adapters) **+ the stats footer** (time/↑/↓/tok-s/model, live from store accumulators then authoritative from the /subagents row, which now carries `model`); **the owner-spec chat composer** (toolbar INSIDE the box: Add Context — Windows multi-file picker (`pick_files` Rust + PowerShell fallback) + project-file picker + @-mention + drag-and-drop → `POST /attachments/read` → chips → attachments ride the send payload and render into model-facing history; permission modes full/ask/plan/editor — migration 0020, `PATCH /sessions/:id/permissions`, enforced in `sessionToolAllowList` (plan = 12 read-only tools, editor = no run_command, full = auto-approve ask-tiers but NEVER the denylist-supreme), children inherit; thinking levels default/low/high/max — per-send `thinkingLevel` → `buildThinkingFetch` injects `reasoning.effort` into chat-completions bodies; the model selector — provider popover with hover flyouts + Manage Models → /settings?tab=api + per-session persisted override; the context donut — `GET /sessions/:id/context` with breakdown/messages/system-prompt/system-tools/memory/meta/mcp + cache hit-rate from REAL `usage_events.cached_input_tokens` (migration 0020, captured from `usage.inputTokenDetails.cacheReadTokens`) + session token/cost totals; empty state centers the composer below mid-screen); **the Models & Providers rework** (independently-scrolling master–detail; catalog multi-select picker with free/paid badges + pre-filled pricing; per-model advanced config incl. input/output/cache-read $/Mtok — plus two real storage bugs fixed: null-clearing never worked, re-upsert reset supportsThinking). Version **0.50.0** across all four manifests + CHANGELOG. **930 tests green** (`pnpm test` = 68 files; was 750/62 — +180, all additive; e2e 12/12, build green, license audit clean 133) + **a live battery against real OpenRouter keys on the real dev DB** (migration 0020 at boot; permissions round-trip; the attachments matrix incl. binary/truncation/escape-block; a thinkingLevel=low+attachment turn — 246 thinking-deltas, the model quoted the fixture verbatim, cachedInputTokens captured; the context report with real numbers; the delegation round — 308 live child frames + battery-r50.md written on disk) — full detail in `docs/ui-iterations/round-50.md` · **What's next:** the owner's re-test (the native browser on his Windows machine — DPI≠100% bounds + multi-tab profile sharing are the flagged risks; the composer's six controls; sub-agent live streaming + stats; the models page); then the bundled installer (ADR-0003/CHANGELOG `[Unreleased]`), a slot-aware `store_provider_key` in the Tauri shell, the 1-line frontend projectId wire-up · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
