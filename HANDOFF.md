...
**Last updated:** 2026-08-29 (round-48 DELIVERED — the owner-test round; every fix traces to the owner's own Windows test session after R47): **sub-agents at near-main-agent parity** (children delegated from a live parent turn now ASK the owner — ask-tier approvals ride the parent's SSE as `subagent-event` envelopes, the approval card appears in the parent chat attributed "Sub-agent CODE · role", decision via the existing POST /approvals/:id/decision; channel-less runs still fail fast; abort signal propagates parent→child with an honest 499 ABORTED outcome; every child carries a deterministic 4-char [A-Z0-9] code on /subagents rows + every subagent-status frame; live per-step tool/text events via chat.ts onStepFinish; the Delegated card shows LIVE clickable per-child rows; the SubAgentPanel is rewritten as a chat transcript in the main chat's visual language with a 1s clock); **the embedded-browser flash loop is dead** (root cause: SessionStore.create() ROTATED the ticket and navigate/viewport/the proxy-adopt path all called it without returning the new ticket → permanent 401 loop at 900ms cadence; now getOrCreate() everywhere, rotation ONLY on POST /browser/session whose response carries the ticket; panel recovery capped at 3/60s then parks with manual Retry; the test mock now mirrors real rotation semantics — lesson #67); **a real files explorer tab** (tree left / content right, markdown + line-numbered code, Search keeps the palette); **sidebar polish** (per-project colors: 8-color least-used palette + migration 0018 backfill of the all-orange legacy rows; new "A_" logo + favicon; collapsed-rail inset-ring fix; Sessions nav removed — route kept for deep links; memory accent bar gone); **the modern always-on-top Windows folder picker** (IFileOpenDialog COM via C# interop owned by a topmost form, classic dialog only as fallback, ERROR: line added; Tauri pick_folder parented to the main window — compile-verified by CI's cargo check only, no cargo in the sandbox). Version **0.48.0** across all four manifests + CHANGELOG. **752 tests green** (`pnpm test` = 61 files, **740 unit + 12 e2e** — split verified against the CI run log; the R47 record was 683/56) — full detail in `docs/ui-iterations/round-48.md` · **CI green: run 33257534609 @ 3336283 (the tip, push event), verified via the Actions API** · **What's next:** the owner's re-test (especially the Windows folder picker + the sub-agent approval flow on his machine); the chat bottom-bar/model-switcher round (owner explicitly deferred it — "let's talk about it later"); the bundled-installer workstream (ADR-0003/CHANGELOG `[Unreleased]`); a slot-aware `store_provider_key` in the Tauri shell (pool keys pasted in the desktop app currently land on the primary key); the 1-line frontend projectId wire-up that turns on per-project cookie isolation · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
