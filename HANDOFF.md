...
**Last updated:** 2026-08-28 (round-45 DELIVERED — "finish the agentic environment": **security round CLOSED the last three deferred audit holes** — P0-3 children never inherit secrets anymore (allowlist `buildChildEnv()` in `agent-core/src/lib/child-env.ts`, applied at EVERY spawn site: exec/git/dialogs tools, both terminal routes, PTY sessions; live child probe prints nulls), P0-4 the AUTO command tier is path-contained (`commandTouchesOutsideRoot()` + `decideCommand(root)` — `cat /etc/passwd` now ASKS; explicit always-allow rules still win), P0-5 web tools gated (`decideWebFetch` host gate on web_fetch + browser_control-navigate: 37-host DEFAULT_WEB_HOST_ALLOWLIST + per-project `web_host_rules` via migration 0016 + interactive approval round-trip where "always" remembers the HOST; web_search stays friction-free but the query is secret-scrubbed before it leaves the machine); **PTY terminal sessions** (round-44 "no interactive stdin" deferral closed: `agent-core/src/terminal-sessions.ts` node-pty engine with persistent-pipe fallback, 6 REST+SSE routes create/list/input/resize/stream/kill with output/exit/error frames, TerminalPanel Run|Shell toggle with ↑↓ history + Kill/New + engine badge, 3/project + 8/global caps, idle reaping, dispose-on-close/SIGTERM — live-verified in the real UI); **packaging v1** (`scripts/release/version.mjs` — version single-sourced at 0.45.0 across root/agent-core/shared/tauri.conf.json, `version:check` gates CI; CHANGELOG.md Keep-a-Changelog 0.37→0.45; `.github/workflows/release.yml` launcher-kit zip + tag-gated draft release — DISPATCHED AND VERIFIED: run 33182499163 SUCCESS, artifact acute-launcher-kit-v0.45.0 24107 bytes); **TOOL_CATALOG drift guard** (`src/lib/tool-catalog-drift.test.ts` — the R44 15-vs-21 lag can't silently recur); **VLM pass fixed 3 real finds** (the R44-c SessionsScreen was ORPHANED — no route imported it; now at /sessions + sidebar nav, lesson #64; NO mobile drawer existed — static 270px sidebar left 69px content at 375px, now an overlay drawer, main width 69→351px; wizard badge hardcoded v0.1.0 → vite __APP_VERSION__ → v0.45.0). **565 tests green** (`pnpm test`, 49 files; was 473) — full detail in `docs/ui-iterations/round-45.md` · **CI green: run 33181641242 @ fd70c5b (push) and run 33184750437 @ 6f75087 (the tip — dispatched via workflow_dispatch after a push-trigger miss), both success, verified via the Actions API** · **What's next:** the owner's verdict on R45; the bundled-installer workstream (the release kit is the launcher flow only — CHANGELOG `[Unreleased]`); memory v2 relevance ranking (digest is still newest-first); e2e coverage for terminal sessions · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
