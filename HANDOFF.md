...
**Last updated:** 2026-09-25 (round-126 DELIVERED — THE CLAY COMPANION REDESIGN: the complete PC UI on the Android app's design language, v0.118.0 → v0.119.0, PUBLISHED. The owner's directive — 'completely redesign the whole UI… go one page at a time… utilize sub-agents and make them work on the things independently… review their work properly' — delivered as the clay foundation (tokens/fonts/the one-time nova→clay migration) + the constitution written FIRST (docs/design-language/ as the PC edition: TOKENS §1d/§10/§11, MOTION on the spring grammar, the NEW SCREENS.md archetypes + navigation laws) + the redesigned navigation model (the project row navigates; the hover-revealed chevron owns the session tree) + TWELVE review-gated sub-agent waves covering every screen (the ~950-line dead-layout retirement, WorkingSection's de-blue — the owner's 'blue colored area' complaint — the settings hub's fifteen tabs, the wizard's kept theatrical DNA, the coherence sweep) + the ~90-spelling drift paydown (audit hex 101→74, arbitrary 1635→1519, JS hovers 21→13). ALL gates green: root tsc + 274 files/4,840 + eslint + design-audit + build + e2e 12/12 + license + docs:check 274/0/0; all five workflows green on 402e1df; v0.119.0 published at /releases/latest with 7/7 assets. The close-out (2faec8f) added THE AI-AGENT-PLAYBOOK (docs/runbooks/AI-AGENT-PLAYBOOK.md — the owner's 'document, plan, and create proper workflows for yourself… so that if any new AI agent apparently decides to work on this project, then that new AI agent will already know and would already have all the necessary details which it would need to perform in a similar way': the behavior laws, the wave protocol, the review gate, the interrupted-run successor pattern) wired into AGENTS.md + the bootstrap prompt + WORKFLOW + ORCHESTRATOR-METHOD. The rollback door: backup/pre-redesign-r126 (pushed at v0.118.0 / 515825c). The owner's own verdict on the shipped UI: 'overall the experience was good and proper, but still it was lacking in a few areas which we can improve properly' — his next walkthrough points the improvement round. The session-by-session history lives in docs/agent/ORCHESTRATION-WORKLOG.md (the R86-era nested prior-state summaries this line used to carry are preserved there).)
...
1. This file, fully.
2. `docs/README.md` (the documentation index) + `docs/runbooks/WORKFLOW.md` (the session spine) + **`docs/runbooks/AI-AGENT-PLAYBOOK.md` (the behavior contract — how agents on this project work: the reading order, the behavior laws, the wave protocol, the gates, the release ritual)** — then `AGENTS.md` (repo root) — the operating rules. If you only have the repo, the same rules are condensed in §5 below.
...
## 3. Exact current state (2026-09-25, round-126 DELIVERED — the Clay Companion redesign; v0.119.0 PUBLISHED)

**R126 is DELIVERED** (the full PC UI redesign round): every desktop screen on the Android app's Clay Companion design language — the clay foundation (the token bridge with 24 `--ac-*` vars + the pattern classes + Manrope/JetBrains Mono + the one-time nova→clay migration), the constitution docs written FIRST (`docs/design-language/` as the PC edition), the redesigned navigation model (the sidebar's project row navigates into the chat; a hover-revealed chevron owns the session tree; one recessed session well), and twelve review-gated sub-agent waves (dashboard, usage, ProjectView, the ~950-line dead-layout retirement, AgentChatPanel, WorkingSection's de-blue, Composer, the right sidebar + SubAgentPanel's de-blue, the settings hub's fifteen tabs, the wizard, the coherence sweep). The method is now documented as the normative **AI-AGENT-PLAYBOOK**. The drift ratchet moved DOWN ~90 spellings. The owner's own verdict: overall good and proper, a few areas to improve — his next walkthrough points them. **The minimum commit to verify from is the R126 close-out `2faec8f`** (on top of the redesign `402e1df`, tagged v0.119.0). Full evidence: `docs/ui-iterations/round-126.md`. The rollback door: `backup/pre-redesign-r126`.

**The next queue (R127+):** (a) the owner's improvement pass on the redesign (his 'lacking in a few areas' + 'improve our design language alongside' — his walkthrough names the areas; the constitution docs take the refinements in the same round that ships them), (b) the owner's device pass on v0.119.0 (the round-126 honest caveats: streaming-state materials, real-sidecar test bands, secondary OS windows), (c) the ZCode adoption queue (D3 round-aligned selection + invalidateRuntimeTokenUsage, D5 the rapid-refill breaker, D4 post-compact re-injection), (d) the standing queue (the menu-timer hover ratchet, the release-flow speed round, multi-PC from the phone, the StablePrefix prompt-cache split). The old R87-era modularity roadmap shipped across R87–R101 (the server split, the turn-loop extraction, the api.ts seams) — see `docs/architecture/MODULARITY-ASSESSMENT.md` §10 + the ORCHESTRATION-WORKLOG for the history.
...
| Tests | `pnpm verify` green at the R126 close: lint + typecheck + root **274 files / 4,840 tests** + build + license audit (CLEAN); agent-core 157 files / 2,903; mobile 48 suites / 1,049; e2e 12/12 vs the built dist; `cargo check` green on CI (Rust Checks workflow) |
...
| **Git remote** | `https://github.com/testplay-byte/ACUTE-CODE` (**PUBLIC** since the owner's R93 directive — unlimited free CI; he may flip it back at will, so every round stays safe in both states: never commit secrets, noreply emails only, no shared-secret strings in docs; verify visibility via the API before your first push). **Fully synced: code tip = `2faec8f` (R126-c the playbook close-out, on top of `402e1df` the redesign, tagged v0.119.0)**, CI GREEN. Clone and confirm `git log --oneline -1` shows `2faec8f` or newer. |
...
## 9. What's next: the R127+ queue (post-R126)

In priority order (the full rationale in `docs/status.json` plan.upcoming):

- **The owner's improvement pass on the redesign** — his own verdict on v0.119.0: *"overall the
  experience was good and proper, but still it was lacking in a few areas which we can improve
  properly"* + *"we can improve our design language alongside with it."* His next walkthrough
  names the areas; the constitution docs (`docs/design-language/`) take the refinements in the
  same round that ships them.
- **The owner's device pass on v0.119.0** — the round-126 honest caveats awaiting live
  confirmation: the streaming-state materials, the real sidecar's test bands, the secondary OS
  windows, and the general feel of the clay material on his machines.
- **The ZCode adoption queue** (the context-compression round): D3 round-aligned selection +
  `invalidateRuntimeTokenUsage`, D5 the rapid-refill breaker, D4 post-compact re-injection; then
  unified tool-result budgets + artifact spill, and cache_control breakpoints for
  Anthropic-format providers.
- **The standing queue**: the menu-timer hover ratchet (21 pairs), the release-flow speed round
  (r110-feedback #8), multi-PC from the phone (the StoredHost[] seam), the StablePrefix
  prompt-cache split (the biggest cost lever).

Historical context: the Phase-3 orchestration engine (SPEC §F3 + ADR-0001) largely SHIPPED
across R36–R79 (delegation with task_id/background/resume, semaphores, the approval engine
ADR-0024, the three run modes); the R87-era modularity roadmap shipped across R87–R101; the
PILLARS §7-4/5/6 pillars (workflows/scheduler/canvas) remain unbuilt and owner-gated. The full
owner vision (three products) is recorded in `docs/architecture/PROJECT-MAP.md` §1/§6.
