...
**Last updated:** 2026-09-08 (round-79 DELIVERED — THE ORCHESTRATOR ROUND: delegate_task task_id/background/resume, the standing R73 deferral — addressable, resumable sub-agent tasks, the last unshipped piece of the R71 sub-agent discipline; no new owner field report this session — the session instruction was continue with planning documentation and backup, done: backup/pre-r79 branches pushed in BOTH repos before any edit): (1) THE TOOL (`agent-core/src/tools/plugins/delegation.ts`) — `delegate_task {task, role?, task_id?, background?, resume?}` with three call shapes: BLOCKING (task only — the pre-R79 behavior verbatim, optionally + task_id), BACKGROUND (task + task_id + background:true — REQUIRES task_id; returns IMMEDIATELY with the receipt: task_id, session id, code, role, model + the resume instruction, while the child runs DETACHED through the same runChildTurn machinery: status frames/notifications/watchdog/turn-registry/key-slot), and COLLECT (resume: task_id | child session id | 4-char code — child running → resume WAITS (~300ms session-status poll, abort-honest "still running" line; the R71 no-polling discipline: resume IS the wait); completed → the final report (idempotent collect); failed → the ADR-0022 §3 retry continuation, awaited; unknown → the honest addressable list); honest refusals everywhere (task_id grammar ^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$, duplicates naming the existing task's status, the ≥10-outstanding background fan-out cap listing them, background-without-task_id, neither→the addressable list) — the tool NEVER throws; (2) THE DELIVERY CHANNEL — a per-turn "## BACKGROUND TASKS" system-prompt section (the taskHints/modeHints ephemeral pattern; prompt-registry section "background-tasks", count 23→24; a cheap indexed guard so childless sessions compose byte-identically; only UNCOLLECTED addressable children, live status lines + the resume instruction, capped at 10 rows) + the NEW delegation.collected event on the PARENT's log (blocking+task_id writes it at completion — inline delivery, never nags; background writes it at resume; skipped by assembleHistory and the frontend fold by construction); (3) PERSISTENCE — migration 0028 (sessions.delegate_task_id + idx_sessions_parent_task; NULL default = pre-R79 behavior, set only through the gated tool path, immutable for the child's lifetime), listSubAgents rows + every subagent-status frame carry taskId; (4) THE FRONTEND — SubAgentStatus.taskId (additive, null = pre-R79 children), the frames' taskId with carry semantics in stream-store, the panel header's mono task chip (subagent-taskid-chip, beside the code chip, tooltip teaching delegate_task {"resume":"…"}) + the chat card's task chip (subagent-card-taskid); the parent-abort cascade is forwarded to detached children (no zombie spend after a Stop; the owner's panel Stop remains THE stop surface). Verification: root 2741/2741 in 145 files (agent-core 1769 incl. the NEW r79-delegation suite's 27 tests; frontend 960 incl. +6; e2e 12), lint clean, typecheck clean, agent-core build ships 0028, scripts/battery-r79.mjs 10/10 live (a dedicated sidecar + the body-dispatch mock provider recording REQUEST BODIES as the model-facing oracle: blocking regression, background outliving the turn ~400ms vs a 4s child, the next turn's request body carrying the reminder section + its disappearance after collection, resume waiting ~2.7s for a 3s child, duplicate refusal, /subagents taskId, the unknown-address list, the failed-child "You were interrupted" retry continuation). **R78 CLOSED: v0.77.0 = commits ca5e714 + close-out ff65a5f, tag v0.77.0 published, CI 34203679389 green, release 384564517 live, DASHBOARD synced (342b7ce).** The R79 release close-out (commit → tag v0.78.0 → Release → publish → CI watch → DASHBOARD truth-sync → status.json ci sync) follows per MAINTENANCE recipe g — the release agent owns it. · **What's next:** the R80 queue: external plugin ctx enrichment (cline's appendContext seam), the lessons-ledger affordance as the reminder injector's third consumer, ratings-driven prompt tuning, the optional v0.68.0 backfill tag, and the standing items (edit-linting, installer code-signing, the Files-tab polish, agent web-app-testing tools) · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
...
## 3. Exact current state (2026-09-08, round-79 DELIVERED — v0.78.0 staged, release close-out pending)

**R79 is DELIVERED: the orchestrator round — addressable/background/resumable delegate_task (the standing R73 deferral) — shipped, unit-tested (2741 in 145 files: agent-core 1769 incl. the new r79-delegation suite (27) + frontend 960 + e2e 12), and live-battery-verified (scripts/battery-r79.mjs, 10/10 with the request-body oracle) — see `docs/ui-iterations/round-79.md` + ADR-0028.** R78 is CLOSED (v0.77.0 = commits `ca5e714` + close-out `ff65a5f`, tag v0.77.0 published, CI run 34203679389 green, release 384564517 live, DASHBOARD synced at `342b7ce`). **The minimum commit to verify from is `ff65a5f`** — the R79 work (agent-core + src + tests + battery-r79.mjs + this round's docs + the 0.78.0 version bump) sits uncommitted ON TOP of it, with the pushed `backup/pre-r79` branch (ACUTE-CODE @ `ff65a5f`) as the recorded safety net; the release agent commits it, tags v0.78.0, pushes, and watches CI (status.json's ci field is the placeholder it syncs). The round-28-era table below is kept as the historical record.

**The next queue (R80+):** external plugin ctx enrichment (cline's appendContext seam), the lessons-ledger affordance as the reminder injector's third consumer, ratings-driven prompt tuning, the optional v0.68.0 backfill tag, and the standing items (edit-linting, installer code-signing, the Files-tab polish, agent web-app-testing tools). Known R79 residuals for the next agent: the sync-path child `turn.error` text stays the generic provider line (pre-existing, out of scope), and there is no parent-model stop of a background task by design (the owner's Sub-agents panel Stop remains the stop surface — the tool description says so).
