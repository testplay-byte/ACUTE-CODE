...
**Last updated:** 2026-09-10 (round-83 DELIVERED, v0.82.0: HONEST TOKEN & CONTEXT METERING — the owner's "highly misleading" round per the token-counting-robustness spec: (1) ONE budget — resolveTurnBudget (models row → catalog → defaults, contextWindowSource LABELED, the owner's per-model max_output_tokens finally honored, available = window − output − 8,000) shared by the context meter, both turn runners, the compaction trigger, the context guard, and the new compact route — the displayed number and the behavioral number are the SAME number now. (2) The honest donut — the `actual` block (the provider's OWN prompt tokens for the last request, from the newest message.assistant stats carrier; null before the first reply, never a fabricated 0) beside the estimate now labeled `~% projected · estimated`; the newest compaction APPLIED to the messages estimate (the ring drops after a compaction instead of lying high) + the 'Context compacted' badge; the budget tick + 'compaction line' note + the window's provenance ('your override' / 'catalog default' / 'assumed 200k — set it in Settings'); schema-MEASURED tool tokens (tools/index.ts measureToolSchemaTokens, replacing the 350/tool guess); the meter's prompt sections match the real turn's (skills, task-modes index, active mode, background tasks, environment). (3) Truth hygiene — migration 0031 (usage_events rebuild: provider_calls + origin + nullable agent_id for the hidden-call rows; DEFAULT 1/'turn' keeps old rows exactly true): 'N turns · M provider calls' (a 5-iteration turn is 1 turn · 5 calls), the compaction summarizer + debug analyst's REAL spend finally billed (origin compaction/debug), cached-NULL hit rates ('— not reported by this provider', never a fabricated 0%), (unpriced) cost markers, 'per model call' chip labels. (4) POST /sessions/:id/compact EXISTS — the affordance the pre-R83 800K guard promised while pointing at a nonexistent command; the guard itself is now model-relative with honest numbers. (5) The stream-store renders meta.compaction/meta.context_limit as live notes + invalidates the meter on done. Verified: root 155 files/2,825 tests (agent-core 1,867 incl. r83-budget + r83-compact-route; frontend 946; e2e 12/12), lint + typechecks clean, license 134 CLEAN, docs:check 187/0/0, live battery 5/5 on the real keys (scripts/battery-r83.mjs — actual=61 tokens measured end-to-end on a real OpenRouter turn). Full detail: docs/ui-iterations/round-83.md · the normative doc: docs/runbooks/CONTEXT-METER.md · spec: agent-ctx/research/token-counting-robustness.md). · **What's next:** the modularity Wave 2 (the server.ts route split, the turn-loop harness extraction, the ToolDeps cycle break), then the standing R81-queue items + the deepseek-harness round-2 candidates (per-tool timeoutMs/TOOL_TIMEOUT, mode-policy-as-event, tool-result spill) · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
