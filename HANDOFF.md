...
**Last updated:** 2026-08-31 (round-57 DELIVERED — the engine-bundling round; the owner's tenth Windows test session was the FIRST CONFIRMED HALF-WIN — the launcher's app-or-site question worked, the SITE launched and ran perfectly end-to-end including the dashboard, exactly as the owner described — but the desktop app still could not reach agent-core, and R54's stderr plumbing finally captured the engine's dying words: `ERR_MODULE_NOT_FOUND` on all three startup attempts): the packaged engine's `node_modules` was a pnpm LINK FARM (measured: 189 symlinks on POSIX = 189 junctions on the windows-latest release runner) that tauri-bundler + NSIS pack/extract does not preserve — the installed engine could not resolve its first bare import and died before its first log line, while every Linux-side check passed (Linux preserves links; the R51 "proven bootable" gate verified the wrong platform and R56's audit never asked "can NSIS extract this?"). FIXED STRUCTURALLY, no product code touched: (1) the staging install now runs `pnpm install --prod --config.node-linker=hoisted` — a classic npm-style tree of REAL directories, zero links, verified with the exact CI pnpm 11.22.0 AND 11.24, staged tree 301.5 MB → 124.6 MB (a yaml-only `nodeLinker:` key is IGNORED by pnpm 11.x — the CLI flag is load-bearing; `node_modules/.bin` is deleted too: POSIX bin shims are symlinks and the runtime tree never spawns bins); (2) a ZERO-LINKS GATE inside stage-sidecar.mjs walks the entire staged tree and FAILS the stage on any symlink/junction (lstatSync reports junctions as symlinks — one walk covers both OSes) plus refuses a package-bearing `.pnpm` — it already caught the yaml-only regression mid-development; (3) THE WINDOWS PRE-PACK BOOT GATE in release.yml: before `pnpm tauri build` can pack anything, CI boots the REAL staged node.exe against `app/dist/main.js` with the same env the Rust shell passes (ACUTE_TOKEN/ACUTE_DB_PATH, cwd = app dir) and polls for the ACUTE_READY handshake (90s) — a failed boot prints the engine's stdout/stderr tails and fails the release; this gate would have caught R53's stale port, R54's silent one-shot, R55's EISDIR, and R57's link farm — every packaged-engine failure to date — before any of them reached the owner. Verification: lint clean, typecheck clean, 1074/1074 in 77 files after the same-round CI-flake fix (a latent bare-setTimeout race — 11 sites now ride the new leak-safe useTimeoutClear hook; the shipped installer was never affected), version:check 0.57.0 ×4, docs:check 147/0 (sandbox 429-WARNs noted), staged-boot battery ACUTE_READY + /health 200 + /api/v1/agents 200 from the staged tree alone. Version **0.57.0**. Full detail in `docs/ui-iterations/round-57.md` · **What's next:** the owner's Windows re-test (double-click ACUTE.bat → the app-or-site question → [1] the desktop app → install 0.57.0 → the console should finally print `✓ agent-core is up — port N` and the window should reach the dashboard — the same flow the owner just watched work in the browser), then the R50 queue (command watchdog polish, sub-agent supervision stats UI, terminal output visibility, usage screen section 2, model-selector hover, plugin tool system), installer code-signing · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
