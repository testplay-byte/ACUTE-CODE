...
**Last updated:** 2026-09-02 (round-64 DELIVERED — the capability round, driven by the owner's first live Windows run of computer use; the 0.63.0 desktop-update flow itself was VERIFIED WORKING by the owner): (1) **THE COMPUTER-USE ROOT CAUSES behind "list_apps → [] / get_app_state('Notepad') → app_not_found / list_displays → []"**: a PowerShell `ConvertTo-Json` pipeline array collapse (0-element arrays emit NOTHING, 1-element emit a bare object — every list-shaped probe silently died), Get-Process-only enumeration (cached MainWindowTitle), and exact-window-title app resolution ("Notepad" never matches "Untitled - Notepad"). Fixed: shape-aware OutJson (`-InputObject @($arr)`), a REAL EnumWindows walk (every visible top-level window + processName per app; list_windows returns all of a pid's windows), a 5-tier app resolver (pid → title → processName → unique substring) whose app_not_found/ambiguous refusals carry a `runningApps` candidate payload, honest empty-result `diagnostics`, the fake 1920×1080 display fallback REMOVED, and the built-in skill rewritten around the visionless UIA-first workflow. All construction-tested (PowerShell never runs in the Linux sandbox) — the owner's next Windows session is the live verification, and the diagnostics keys make any residual failure readable. (2) **THE ALWAYS-ON-TOP FLOATING MONITOR** (owner: "very minimal… clean… beautiful… shown at the top of each and every single one of the screens… automatically show up as soon as the agent starts"): a 360×96 frameless OS window (src-tauri/src/mini.rs — always_on_top, skip_taskbar, focused(false), top-right of the active monitor; mini.html is a THIRD vite entry with its own tiny bundle that polls /computer-use/session 1s and self-closes when the session ends; the main app's ComputerMiniWindow is now the auto open/close CONTROLLER) + the same minimal bar as an in-app pill in web mode; the right-sidebar Computer tab is REMOVED (persisted tabs migrated away). (3) **THE CHAT POLISH BACKLOG**: full markdown formatting (ChatMarkdown — headings/lists/tables/quotes/links/bold/italic, streaming-friendly, path pills kept; applied to final + intermediate + live text), intermediate assistant text never hides in the collapsed WorkingSection, the context-window popover is a measured fixed-layer portal (no clipping, refined border) that updates LIVE (liveTick + 2.5s refetch while streaming), internal notifications auto-dismiss at 1.5s (permission/failed stay), and each Delegated row claims exactly its own sub-agent. (4) **PICKERS/PERMISSIONS/KEYS/TOKENS**: the model picker lists ONLY the models added in Models & Providers; permission-mode changes re-read live (the approval deps' permissionMode is a getter — a mid-turn ask→full flip applies to the NEXT gate); the safe-command auto list grew the Windows/PowerShell read-only reads + search tools; per-API-key usage stats (migration 0024 key_slot + the /usage/detailed keys aggregate + the Usage screen's API-keys cards, honest for unused/removed keys); the token estimator is a GPT-style BPE approximation (CJK/digit/punct-aware; calibrated against documented cl100k ratios). Verification: root 1664/1664 in 109 files (was 1542/104), agent-core 885/885 in 52 files (was 801/50), lint/typecheck/docs:check clean, LIVE browser battery (zero console errors; onboarding → dashboard → usage → chat; mini.html honest in web mode). Version 0.64.0, TAGGED + RELEASED (the R63 discipline). Full detail in `docs/ui-iterations/round-64.md` · **What's next:** the live Windows verification of the R64 computer-use fixes (the runbook checklist), then the standing queue — multi-agent support, packaging/code-signing (SmartScreen), ratings-driven prompt tuning · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
