...
**Last updated:** 2026-09-11 (round-88 DELIVERED: the floating to-do list round — the owner's spec + manual edits, v0.85.0 → v0.86.0. The FLOATING TODO WIDGET (TodoFloat.tsx): top-right of the chat window, collapsed = ONLY the task at hand + N/M, one click expands to the ten-row scroll box (done = checked off, in-progress = pulsing, pending = dimmed). The MANUAL EDITOR: the pencil opens content/status-cycle/reorder/delete/add; Save POSTs the FULL snapshot to the NEW routes/todo.ts (POST /sessions/:id/todo — writeTodo's exact validation ladder + the owner-only EMPTY clear; source:"user" events; frames fan to live turns via the turn registry). The CURRENT TODO LIST prompt section (registry id todo-list, 24 → 25 sections — the versioned pins moved): prepareTurn injects the latest snapshot every turn via latestTodoSnapshot (storage/sessions.ts — an empty array IS the cleared state), with the OWNER-edit emphasis line on user-source snapshots. The agent now always knows its plan state. R87's close-out fix (7b6e47e) also landed: the two Windows-EPERM test suites + the kit's stale credentials copy. Full pipeline green: agent-core 95/1,882 + frontend 62/960, lint/typecheck/build/e2e 12/12.) · **What's next:** the R86-era queue unchanged — Wave 2-b (the turn-loop harness, risk #1) + the remaining 51-route server.ts split, then Wave 3 (the frontend seams; deleting the dead sessions/ dir) · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
