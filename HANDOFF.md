...
**Last updated:** 2026-08-26 (round-42 DELIVERED — desktop notifications with the app window CLOSED: Service Worker + Web Push (VAPID keys at `.dev/vapid.json`, migration 0012, subscribe endpoints, bus fanout), streamed turns now SURVIVE client disconnects (background completion + notification; explicit `POST /sessions/:id/stop` with a distinct ABORTED outcome), task_complete fires for every completed turn, auto-rename recognizes the sidebar's `New chat · <Project>` seed (the actual R41 bug), boot sweep no longer kills idle conversations (last-event assistant → queued); launcher auto-opens the browser after the servers are ready (acute_launcher.py ONLY — .bat/.sh/credentials untouched); the embedded browser panel in web/launcher mode opens URLs in a new tab of the current browser; the notification bell popover is fully viewport-clamped (both axes); sub-agent panel redesigned (10-line clamped task/report + timeline + files manifest); main-chat user bubbles clamp at 10 lines with an expand toggle; chat window min-width 480px with the right sidebar auto-shrinking smoothly; left-sidebar polish (gradient project tiles, count-chip headers) + the activity animation moves onto a collapsed project row/rail tile. **262 tests green** — full detail in `docs/ui-iterations/round-42.md`) · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
