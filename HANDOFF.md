...
**Last updated:** 2026-09-06 (round-69 DELIVERED — the computer-use ENFORCEMENT LAYER, closing the residuals R68's own close-out named; no new owner field report; the round's code + docs are complete, committed and pushed — CI watched to green; NO tag, NO release, NO DASHBOARD sync: those are the next session's close-out steps): (1) **EVERY MUTATING ACTION RETURNS AN OBSERVATION RECEIPT** — the 11 action tools (left/right/double/triple/middle click, scroll, type, key, set_value, select_text, left_click_drag) attach `observation: {frameId, screenChanged?, focusedElementName?, activeApp?:{pid,title}, titleChanged?}` to the SENT receipt after a 600 ms settle (a fresh registered provenance-"observation" frame + raster; screenChanged = full-frame aHash hamming >4 vs the pre-action frame; focusedElementName = the key tool's readback; activeApp/titleChanged = one list_apps capsule, pre+post; a failed capture yields `{captureFailed:true}` — the action receipt never fails on it); `returnState` on all 11 tools: "compact" default / "none" opt-out / "full" the R61 UIA compose — the verification read the model used to buy with a 5–25 s screenshot+vision round now rides the action receipt (the #1 field failure, screenshot-after-action spam, is structurally unfed). (2) **CLICKS VERIFY WHAT THEY HIT**: targetVerificationStatus gains "changed"/"unchanged" (coordinate clicks upgraded from blind "unverified" by the observation's screenChanged) and the hit-test that already ran now rides the receipt as `hitElementName`. (3) **STALE FRAMES AUTO-REFRESH** (frame_stale is dead for pointer tools): a coordinate action on a >30 s frame captures the old coverage, registers provenance-"auto_refresh", and proceeds if full-frame hamming ≤8 (screenStable) or target-region (element bounds, else ±48px) hamming ≤6 (targetRegionStable) — else the NEW `frame_changed` refusal carries the ALREADY-REGISTERED `refreshFrameId` (one zoom round-trip); set_value/left_mouse_down keep the honest hard fail; capture-fail/unhashable falls back to frame_stale. (4) **THE SPAM GUARD**: the 3rd consecutive model capture with full-frame aHash hamming ≤4 (no intervening mutating dispatch) refuses the NEW `screen_unchanged` BEFORE registration (act / wait() / find_elements guidance); resets on mutation, a changed capture, or a foreground-pid flip; auto-refresh + observation captures never count. (5) **FRAME-HASH INFRA**: NEW `agent-core/src/computer/framehash.ts` (pngjs@7.0.0 MIT — 134 prod deps, audit CLEAN; 8×8 Rec.601 aHash, region crops, 4-entry decode LRU) + `FrameInfo.aHash/provenance` (model|auto_refresh|observation). (6) **wait() IS OBSERVABLE** (post-sleep observation; actionSent stays false) and **middle_click/right_click take element targets** (raw center click — open-in-new-tab without coordinates; double/triple keep the honest refusal). (7) **SSE**: the refresh frame emits `{type:"screenshot", tool:"auto_refresh"}` then the observation frame `{tool:<action>}` — post-action thumbs render INLINE in the chat at the capture moment; frontend UNTOUCHED. (8) **WINDOWS INPUT ENGINE**: the scroll tool was QUADRATIC (N events × N×120 — now ONE event, ticks×120, horizontal = real MOUSEEVENTF_HWHEEL 0x1000, SetCursorPos first); long type >300 chars rides `psStdinCapsule` (script stays in -EncodedCommand argv, TEXT rides STDIN as base64 UTF-8 → Set-Clipboard → 200ms → Ctrl+V — the 32,767-char ARGV ceiling bypassed with honest empty/corrupt refusals); the Chromium poke is DUAL (WM_GETOBJECT OBJID_CLIENT 0xFFFFFFFC + UiaRootObjectId 0xFFFFFFFD) with a 400/800/1400/2000ms sparse-poll, and msedge/chrome launches append `--force-renderer-accessibility`. (9) **THE MINI MONITOR STOPS STEALING FOCUS**: tao 0.35.3's set_focus synthesizes an ALT-key SendInput pair (stray synthetic input mid-agent-action) — the re-open call is DELETED and the window carries `WS_EX_NOACTIVATE` (0x0800_0000 via GetWindowLongPtrW/SetWindowLongPtrW, zero-read guard protects TOPMOST; NO WS_EX_TRANSPARENT — the STOP button + drag region stay interactive). (10) **THE TEACHING**: prompts.ts CHAIN DISCIPLINE rewritten (read the receipt — no screenshot/zoom after acting; unchanged → element-first/adjust; wait() after navigation; screen_unchanged semantics), skills.ts re-taught to match (the R68 zoom-crop loop retired), tool descriptions + shared returnState schema; golden fixture regenerated (+4/−2). Verification: root **2082/2082 in 123 files** (was 2003/122; agent-core 1180/60, frontend 890/61 unchanged, e2e 12/12), lint/typecheck exit 0, build ✓, cargo check (x86_64-pc-windows-msvc) ✓ zero mini.rs warnings, docs:check 0 failures (the eight future-dated R68 stamps re-dated to the real UTC review date), version 0.69.0 ×4. Full detail in `docs/ui-iterations/round-69.md` · **What's next:** (a) the DASHBOARD truth-sync to 0.69.0 + tag v0.69.0 + the GitHub release (the close-out ritual — deliberately NOT this round), then (b) the owner's live Windows verification — R68's 7-step Edge flow PLUS the R69 live gates (dual poke on real Edge, the stdin paste channel, HWHEEL feel, the non-activating monitor, the aHash thresholds on real frames, the 600 ms settle) — all Windows behavior remains construction-pinned (PowerShell never runs in this sandbox) · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
