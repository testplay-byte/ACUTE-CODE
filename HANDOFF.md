...
**Last updated:** 2026-08-27 (round-44 DELIVERED — "complete the whole agentic coding environment": **agent memory system** (migration 0015 `memory` table + `memory_save`/`memory_recall`/`memory_list` tools 20-22 + newest-slice digest auto-injected into every project-turn system prompt ~1500 chars + right-sidebar **Memory tab** + `GET/DELETE /projects/:id/memory` read-and-prune routes — see `docs/runbooks/PROJECT-MEMORY.md`); **REAL web search** (DuckDuckGo html + lite chain with honestly-labeled MediaWiki last resort, 8 results, zero deps/key, live-verified); **session search/fork/revert** (`GET /sessions?q=` LIKE over title+event payload, `POST /sessions/:id/fork` copies the event log, `POST /sessions/:id/revert {keepThroughSeq}` truncates + `session.reverted` marker; SessionsScreen debounced search + hover Fork, chat "Revert to this message" with confirm); **sub-agent keys from credentials.txt** (launcher parses optional `OPENROUTER_SUB1..3_KEY`, auto-appends baked-in defaults, distributes to keyring pool slots 2/3/4 via `ACUTE_PROVIDER_OPENROUTER_SLOT{2,3,4}` — Settings shows them with zero manual entry; `scripts/dev.mjs` `readSlotKey` parity); **streaming terminal** (`POST /projects/:id/terminal/stream` SSE stdout/stderr/exit/error frames + `: ping` heartbeats, body `{timeoutMs,maxBytes}` can only SHRINK the 60s/64KB budgets, client-disconnect kills the child; TerminalPanel live chunks + Stop + exit-code footer + sync-route fallback); **VLM UI pass** (12 screenshots + vision analysis caught 2 real bugs: dashboard recent-activity cards dead-linking `/sessions` → now `?session=` deep links, and the R43 Sub-agents settings tab being unreachable — it was in SettingsPage TABS but not the sidebar's SETTINGS_SECTIONS; plus phantom slot-1 row dropped, terminal hanging indent, provider hover endpoints); **docs + maintainability** (`docs/runbooks/MAINTENANCE.md` — architecture map + six how-to-add recipes + golden rules; `PROJECT-MEMORY.md`; IMPLEMENTED-API ROUND-44 section; TESTING/SETUP/WORKFLOW refreshed). **471 tests green** (`pnpm test`, 44 files, incl. 8 e2e; was 397) — full detail in `docs/ui-iterations/round-44.md` · **CI: green on wave 1 (run 33038236165 @ 8156bde) but RED on the tip** — `terminal-stream.test.ts` asserts `"hello\n"` where windows-latest emits `"hello\r\n"` (CRLF); **first task of the next session: fix that assertion, push, watch CI to success** (golden rule 3) · **Also open:** `TOOL_CATALOG` in `src/lib/api.ts` is stale (15 of 21 tools — the agent dialog's tool checkboxes lag), DASHBOARD not yet truth-synced to R44, security holes P0-3..P0-5 still deferred by the owner · **What's next (owner verdict on R44, then):** the CRLF test fix, TOOL_CATALOG sync, DASHBOARD truth-sync, packaging/versioned releases, PTY terminal, security round (P0-3..P0-5) · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
