...
**Last updated:** 2026-08-31 (round-58 DELIVERED — the desktop-polish round; the owner's eleventh Windows test session was the FIRST FULLY-WORKING desktop run in the product's history — the R57 engine-bundling fix held: *"This time the application works properly. The app opens up without any errors or any issues at all."* — and the session's precise defect list became this round, ALL fixed): (1) the native window title bar is GONE (decorations:false + a 40px integrated in-app TitleBar — drag region, app identity, min/max/close, null in web mode; window-control permissions verified against the ACL manifest); (2) the pop-out browser window is no longer a white box (the SYNC Tauri command deadlocked webview creation on Windows — now async like browser_tab_create; the nav overlay rides initialization_script so it survives page loads; the pop-out shares the ONE browser profile — the old "isolated" tooltip lied); (3) "open in system browser" works (new open_external_url Rust command via the shell plugin's Rust-side ShellExt — bypasses the JS ACL); (4) the URL bar no longer resets mid-typing (focus-guarded draft sync) and clamped viewport readouts say so honestly; (5) STOP is honest: the engine flushes the partial segment, records completed-iteration usage, resets the session to queued (the eternal sidebar spinner), and returns "stopped by user"; the client classifies its own deliberate abort (no more "Generation failed / body stream buffer was aborted"), renders a quiet TurnStoppedCard, and a new Continue button resumes from the persisted partial + tool results; (6) LIVE FILE-WRITE PREVIEW: the AI SDK's tool-input-start/delta parts (the model's JSON args streaming BEFORE the call completes) now flow chat.ts → runtime → SSE → stream-store → a "writing <file> — N chars" partial-content box (tolerant incremental JSON extractor in streaming-args.ts), replaced by the diff on completion — battery-verified on the owner's exact 3-file task (40-83 arg deltas during generation); (7) the end-of-task session REGURGITATION is capped (assembleHistory previously replayed every tool's full 4k-char output as user-role blocks on every outer iteration — the last 8 results keep full fidelity, older stub to 200 chars, blocks bounded at 48k, and the completion-signal list grew the phrasings real summaries use); (8) the thinking blocks lost the AI-glow accent rails (clean self-contained notes blocks, main + sub-agents); (9) the context popover waits ~600ms of hover intent (focus/click instant); (10) Settings restructured: unconfigured preset providers collapse into a "Not configured" group, preset providers hide base-URL/API-format, the provider list is content-sized with a 220px floor, disable is a proper switch with an agent-impact confirm, the OpenRouter catalog no longer leaks into custom providers, Sub-agents is the single home for keys/model/parallelism/supervision (Advanced keeps connection + memory — the double-render is gone), the chat ModelSelector honors hidden/displayName config, and EVERY stored API key is revealable (new POST /providers/:id/keys/reveal — the owner's explicit demand, consciously reversing R47's no-keys-in-responses invariant for this single authenticated route); (11) the CLI CHAT HARNESS: scripts/acute.mjs grew chat:new/chat/chat:stream/chat:stop/chat:events/chat:ctx/approvals — live SSE turns with thinking/tool/approval rendering, Ctrl-C-to-stop, --auto-approve, --raw JSONL (docs/runbooks/CLI-HARNESS.md) — long sessions are drivable end-to-end from the terminal. Verification: root suite 1169/1169 in 80 files, agent-core 631/631 in 38 files, lint/typecheck CLEAN (root + agent-core), docs:check 149/0, version:check 0.58.0 ×4, LIVE battery 5/5 (scripts/battery-r58.mjs — the permanent R51-pattern artifact: dedicated sidecar, the owner's 3-file task with frame-order checks, stop → stopped-frame + queued + persisted partial, continue completes, reveal returns the real key), browser verification of the restructured settings pages (agent-browser, zero console errors), Rust changes stub-crate cargo-check'ed locally against the real tauri 2.11.5 + plugin-shell 2.3.5 sources + CI cargo check on push. Version **0.58.0**. Full detail in `docs/ui-iterations/round-58.md` · **What's next:** the owner's Windows re-test of 0.58.0 (title bar, browser pop-out + system-browser open + URL typing, stop/continue, live write preview, settings pages), then the prompt-section registry (modular prompt overrides — the .acuterules extension point grows into per-section files, the deepseek-harness modularity reference), a long-session CLI soak (the harness now exists), the R50 leftovers (usage screen section 2, plugin tool system polish), installer code-signing · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
