...
**Last updated:** 2026-09-09 (round-82 DELIVERED, v0.81.0: MODELS & PROVIDERS HARDENING — the owner's field report round 2 per the models-providers-fixes spec: (1) THE custom-provider routing fix — body.providerId rides both send routes, queued messages, the vision relay, the context meter (?providerId=), and the debug analyst; prepareTurn resolves the EFFECTIVE provider, so a model picked under a custom gateway or NVIDIA serves there (the 'No endpoints found' misroute is gone; unknown ids 400 honestly). Sub-agent children route the same way: orchestration.subagentModel is now the provider-scoped {providerId, modelId} ref (legacy strings read openrouter-scoped) and runChildTurn/retryChild provision the child's key pool + keyring VIEW from the override's provider. Custom-provider keys SURVIVE RESTARTS (keys.rs notes ids in ~/.acute/custom-providers.txt and re-injects ACUTE_PROVIDER_<ID> every spawn — cargo check green). (2) POST /models/:id/test — the per-model TEST button: a real minimal completion probe (http/auth/modelAccepted/nonEmptyContent checks, latencyMs, scrubbed contentPreview, provider-reported usage, honest reason; HTTP 200 either way), on every configured row + inside the edit dialog. (3) The edit dialog's capabilities card: Reasoning/Vision On/Off + Tool use/Audio input/Video On/Off/? tri-states (migration 0030's nullable columns — NIM/custom rows start UNKNOWN, never the 0004 false-lie; the INSERT path's null→false fold fixed). (4) NVIDIA polish: the Add-models picker's Free/All toggle is dialog-LOCAL (never writes the composer's shared pref) and auto-switches to All for zero-free catalogs; GET /models/configured feeds the sub-agent card's per-provider rows; the running override shows its providerId·modelId chip. Verified: root suite 2,809 (agent-core 1,853 incl. four new r82 suites; frontend 944; e2e 12/12), lint + typechecks clean, license audit 134 CLEAN, docs:check 186/0/0, and a LIVE battery 8/8 on the real keys (scripts/battery-r82.mjs: a custom-gateway turn routes through the provider's OWN endpoint+key via a local mock oracle; the real OpenRouter + NIM probes return honest verdicts; a hostile key-echo gateway is scrubbed). Full detail: docs/ui-iterations/round-82.md · spec: agent-ctx/research/models-providers-fixes.md). · **What's next:** the R83+ queue — honest token/context metering (the token-counting-robustness spec: the provider-reported 'actual' block, the estimator fed the real turn context, compaction-aware metering, the unified resolveTurnBudget, the missing /compact the 800K guard already promises), then the modularity Wave 2 (server.ts route split) · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
