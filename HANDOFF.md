...
**Last updated:** 2026-09-06 (round-70 DELIVERED — THE AGENT BRAIN ROUND, research-driven (the round's own prompt audit R70-A + the OSS agent study R70-B — no new owner field report); the round's code + docs are complete, committed and pushed — CI watched to green; NO tag, NO release, NO DASHBOARD sync: those are the next session's close-out steps): (1) **ENV GROUNDING** — every turn's prompt states the actual OS + release, the real shell run_command spawns (cmd.exe on Windows / /bin/sh on POSIX — the TERMINAL section teaches ONLY the real platform's syntax now, was: both always), the current date, and the git branch + dirty state (runtime.ts probeGitState, a 1.5 s guarded probe with honest not-a-repo / probe-failed fallbacks; a dirty tree is named the USER's work the agent must never revert). (2) **PROMPT CONSOLIDATION** — agentic-loop + efficiency + task-planning + todo-tracking (4,762 chars, the same posture four ways) merged into ONE five-phase AGENTIC LOOP (PLAN → EXPLORE → ACT → VERIFY → FINISH; todos for 3+ steps, skip trivial; a delegate_task adversarial review for 3+ file edits; maxOuterLoops taught honestly); 3 registry ids retired via the R66-2-c cascade (23 → 20 ids); the golden fixture regenerated by the sanctioned procedure (22,625 → 20,156 bytes). (3) **AGENTS.md/CLAUDE.md CONVENTIONS LOADING** — prompts.ts readCustomRules reads AGENTS.md → CLAUDE.md → .acute/rules/*.md → .acuterules → AGENTS.override.md → CLAUDE.local.md in presentation order (later = more specific) with source markers; @file imports (relative, .md/.txt, 4 max, cycle-guarded); caps 16K/file + 32K total; the custom-rules prompt section documents the hierarchy — every convention-rich repo opened in ACUTE-CODE now has its instructions honored (the checked gap: Codex/Claude Code/Cursor/OpenHands converged on AGENTS.md). (4) **EDIT + VERIFICATION DISCIPLINE** — the line-number-prefix anchor rule (read_file is cat -n since R70-a), the Codex dirty-worktree rule (never revert user changes, STOP on unexpected modifications, never git reset --hard / checkout -- / clean), prefer-editing-over-creating, and the verify-after-edit contract (run the touched tests/typecheck/lint; discover commands from AGENTS.md / package.json; ask the owner once + memory_save). (5) **COMMUNICATION CONTRACT** — under 4 lines by default, zero preamble/postamble, path:line citations (`src/app.ts:42`), final reply = what was done + verification evidence + next step. (6) **TOOL FEEDBACK QUALITY (the ACI round — SWE-agent's "observations quality ≈ model quality")** — run_command's 64KB output cap now keeps HEAD 32KB + TAIL 32KB with an honest middle-omission marker (build/test errors live at the END of logs — the tail was previously LOST); read_file gained offset/limit pagination + cat -n line numbers + honest empty-file/EOF messages + line-aligned head+tail for >256KB windows (the RAW readFile for the UI viewer untouched); todo_write's description carries the Codex planning discipline; silent successes say "(no output — the command ran successfully and printed nothing)"; job_status's empty-log-file bug fixed. (7) **THE SKILLS SYSTEM ROUND** — file-based skills (project `.acute/skills/<name>/SKILL.md` + user-global `~/.agents/skills/`, the Agent-Skills standard frontmatter, DB-row precedence with files read-only via 409 refusals, GET /skills merged listing with provenance); SEVEN new builtin bodies (code-review 1,362 · debugging 1,398 · testing 1,138 · git-workflow 1,298 · web-research 1,185 · project-init 1,468 — the /init skill that WRITES the project's AGENTS.md from codebase analysis · browser-use 1,822 — absorbing the browser-panel craft); STICKY skill bodies (read_skill + memory_recall persist with a 60K budget — was the 4K head+tail mangle — and skip the 200-char replay stub; last-resort 8K + reload marker; loaded instructions no longer evaporate mid-task); the computer-use skill gated on the master switch (dark tools no longer advertised) + read_skill added to PLAN_MODE_TOOLS; agent.skills WIRED (the per-agent allowlist was dead data since R61 — non-empty = allowlist over builtin + user + file skills, via the ONE shared resolver both the prompt index and read_skill use). (8) **PROMPT SIZE: 22,460 → 20,156 chars** while ADDING grounding + conventions + discipline (browser-panel 4,087 → 2,549 + computer-use 3,934 → 3,259 + the consolidation −1,927; the deep craft moved into the sticky skill bodies — the prompt teaches the loop, the skills carry the craft). Verification: root **2177 tests in 126 files, all green** (the 12 sidecar-e2e ran in-suite against the fresh build; agent-core **1275/1275 in 63 files** — +95 this round: r70-tool-feedback 18 + r70-skills-system 41 + r70-prompt-round 34 + 2 pin updates; frontend 890/890 unchanged — zero frontend files touched; e2e 12/12 vs the built dist), lint/typecheck exit 0, build ✓, license audit CLEAN (134 deps, no dependency changes), version 0.70.0 ×4. Full detail in `docs/ui-iterations/round-70.md` · **What's next:** (a) the DASHBOARD truth-sync to 0.70.0 + tag v0.70.0 + the GitHub release (the close-out ritual — deliberately NOT this round), then (b) the owner's live gates — the R69 Windows checklist PLUS the R70 questions (which of the seven new skill bodies need tuning on real tasks, whether AGENTS.md conventions load on the owner's real repos, whether the grounded TERMINAL syntax kills the cross-platform command guesses) · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
