...
**Last updated:** 2026-09-17 (round-101 DELIVERED: the twelfth-walkthrough round, v0.98.0 → v0.99.0 — THE CALM UPDATE HAND-OFF + THE CHAT TIMELINE + HONEST MERMAID + LINUX ARM64: the v0.98.0 'environment may be running or crashed' report answered at the ROOT (the pre-install kill was deliberate but UNANNOUNCED — the watchdog honestly diagnosed a crash and the offline screen owned the final seconds; now run_update_installer emits update-installing BEFORE the kill, the updateInFlight store flag suppresses the watchdog/connect-loop offline flip, the ConnectionGate renders the calm 'Restarting into vX — your data is kept, the app comes back by itself' splash with the children unmounted so NO query banner can flash, and a REJECTED launch self-heals via auto-retryConnection — the NSIS /R relaunch itself was verified-correct all along); THE CHAT TIMELINE RAIL (every transcript item rides a 28px rail + node dot on a gradient-faded continuous spine — solid=you/hollow=agent/red=error/queued=muted, punch-out rings, sr-only labels + title timestamps — plus the composer's focus ring DELETED for a border-color-swap-only :focus-within, the 'selected the message entry area and the area around it' complaint); THE SIDEBAR RAIL POLISH (CSS-only RailLabel chips on hover/focus with full project names, the 2px accent bar on active rail nav, the +N overflow tile, the settings nav pane; the enabling overflow-x-visible/overflow-y-clip geometry); MERMAID HONEST AND RESILIENT (errors surface — console.warn + a detail line in the amber note with the cause-chain walked; one bounded retry on the dynamic import AND the render with fresh ids; thinking/work-section fences now RENDER as diagrams — the D4 defect; check-mermaid-chunk.mjs gates every build on the lazy chunk shipping, proven live at 667.8 KiB); THE LINUX ARM64 RELEASE (linux-bundles-arm64 on the native ubuntu-24.04-arm runner — step-for-step twin of the x64 job, _arm64.deb+AppImage wired into github-release; rust-linux-arm64 CI compile gate ALREADY GREEN on the first push; node-pty's no-Linux-prebuilds truth documented, the stale comments fixed). Full pipeline: lint, typecheck, root 3,818/3,818 (12 env-skips), design:audit clean at the re-pinned-down baseline, docs:check 220/0/0, version ×4 at 0.99.0. The owner's TEST CHECKLIST lives in round-101.md §5. The owner's UI REFERENCES arrive next round — the visual language continues against them.) The prior state: round-100 (the prior-state summary preserved in the round-100.md header): round-100 DELIVERED: the eleventh-walkthrough round, v0.97.0 → v0.98.0 — THE HONEST BROWSER + THE COMPLETE UI OVERHAUL + THE LINUX RELEASE + THE LIVE-FIRE AGENT: the fixedRuntime bundle RETIRED (installer ~258 MB → ~37–40 MB, downloadBootstrapper restored, the CI cab-fetch + staging deleted) with the panel DE-BRANDED (PANEL_USER_AGENT on every content webview — no Edg/ token, AcuteBrowser/1.0 identity, platform-honest Linux string) + the honest ENGINE LINE (Settings → Browser + About) + the trilemma on record (docs/research/browser-engine-and-linux-round-100.md); THE DESIGN FOUNDATION (TOKENS.md revised to the measured ladder — 10px floor, the weight law, 5-step radii, the density table — + the DESIGN-AUDIT GATE scripts/design-audit.mjs wired into verify + CI with the ratchet baseline, + the Kicker/SectionCard/SettingsRow primitives); THE CHAT WINDOW OVERHAUL (research §C4 end to end: type snapped, lucide status glyphs, radii 4/8/12/16/999, the user-message input-row idiom, Send = the 28px no-glow circle, SEAM_GAP 4, the global :focus-visible rule, tabular-nums); THE SETTINGS REBUILD (the VS Code nav-column + search shell, all nine tabs swept — ModelsProvidersTab de-offendered); THE SHELL DENSITY (32px sidebar rows, 240px sidebar, 48px rail) + THE WORKING-SCREEN DE-COSTUME (the dashboard/usage heroes, StatCard, the right sidebar's one toolbar grammar); THE LINUX RELEASE (ADR-0031 the Secret Service key store via the keyring crate target-gated, the rust-linux CI cargo-check job, the linux-bundles release job: deb+AppImage with the pinned linux sidecar + Node + the bash boot gate, README Linux/troubleshooting); THE LIVE-FIRE AGENT BATTERY (7 legs on real OpenRouter turns — tools/memory/approvals/browser verified with disk truth — which caught + fixed the 'Invalid JSON response' instant dead end: now malformed_response/transient + pinned + re-verified live). App-wide measured cleanup: sub-10px text 101→0, heavy weights 474→19, JS hovers 243→33, inline hex 254→103 — enforced by the audit gate. Full pipeline: lint, both typechecks, root 3,785/3,785 (12 env-skips), docs:check 218/0/0, version ×4 at 0.98.0. The owner's TEST CHECKLIST lives in round-100.md §5.) The prior state: round-99 (the prior-state summary preserved in the round-99.md header): round-99 DELIVERED: the tenth-walkthrough round, v0.96.0 → v0.97.0 — the RESEARCH-DRIVEN UI redo + the self-contained app: THE BROWSER SHIPS WITH THE INSTALLER (the WebView2 Fixed Version Runtime via fixedRuntime + the CI-staged webview2-runtime resource — the app never touches the device's engine) + EVERY IN-APP LINK ROUTES THROUGH THE CENTRAL OPEN-LINK ROUTER into the embedded browser panel by default (the chat links were silently dead before) + the Link-opening preference; THE ONE-CLICK SILENT UPDATE (/S /R via ShellExecuteW — the tauri NSIS template's own .onInstSuccess relaunch leg brings the app back; byte-true progress, inline release notes, the 24h startup auto-check + the Sidebar dot + one toast, the wizard fallback); THE CHAT VISUAL REDO (turn headers with model identity, the folded Completed-N-steps summaries, tool-row outcome glyphs, one-line stats footers, the soft streaming caret, the empty-state pill-cards, tabular-nums discipline — the compressed-vs-full matrix documented); THE CONTEXT POPUP REBUILT after the /context reference (the overview hero, the composition bar with inline % labels + the legend with jump-to-management links, one-row cache — both render legs one payload); THE DATA & STATISTICS POLISH (the reorder, one quiet Agent health section, the danger zone always last, the anti-jitter kit); THE PROJECT-WIDE SYSTEM PROMPT MANAGER (master-detail-lite: searchable bucket-grouped list, status chips, token estimates, registry descriptions, Revert all, the promoted composed preview); THE SYSTEM-PROMPT OVERHAUL (the precedence ladder, the autonomy risk tiers, INTAKE phase 0, tool descriptions, the subagent report contract, confidence tags, memory save/recall discipline, the numbers audit — the dedup paid ~2.3K chars; 23,975/24,000, 32 registry sections). THE REVIEW DEBT PAID: a full independent subagent review pass audited the round (ship-shape after 4 MINORs — all fixed + pinned). Full pipeline: lint, both typechecks, root 3,754/3,754 (12 env-skips), agent-core 2,384/2,384, docs:check 213/0/0, version ×4 at 0.97.0; the fixedRuntime chain CI-proven (CI 329126c GREEN incl. cargo check; the Release dispatch on 34fe1e5 SUCCESS — the engine-bundled installer built end to end). The owner's TEST CHECKLIST lives in round-99.md §5.) The prior state: round-98 (the prior-state summary preserved in the round-98.md header): the ninth-walkthrough round, v0.95.0 → v0.96.0 — the owner's v0.95.0 report answered end to end: THE DESIGN LANGUAGE (docs/design-language/ — TOKENS/MOTION/COMPONENTS/WIZARD-DNA/WINDOW-CONTROLS/USAGE: the enforceable layer every UI edit refers to first), THE DATA & STATISTICS SURFACE (the settings tab + the usage screen: totals, the peak day, the 12-month heatmap, the per-model color-coded stacked chart + donut — same model name = one color everywhere, agent health, total cost, the clear-all-data with the exact enumeration), THE DESKTOP NOTIFICATIONS (tauri-plugin-notification; task complete/failed/permission-needed reach the user's PC on the visibility rule; the settings toggle default ON), THE PROMPT CUSTOMIZATION UI (the Prompts tab: the per-project section overrides with the live composed preview, the 8K cap, the drop warning, the diagnostics) + THE ALWAYS-LOAD SKILL TIER (pin any skill — its FULL body rides every turn, budgeted 24K; the SkillsTab switches + the budget readout), THE TOKEN-OPTIMIZATION PAIR (the session file ledger — edit-after-edit without re-reads, staleness warnings; the tiered prompt rule, the golden re-pinned) + search_symbols + THE AUTO-INDEX (the background keeper + incremental reindex after writes), MERMAID IN CHAT (ADR-0030's honest license path — mermaid@11, the two narrow audit pins; lazy, strict, theme-aware, graceful fallback), THE WIZARD'S CUSTOM OPENAI-COMPATIBLE PROVIDERS (the inline create flow), THE BROWSER SCREENSHOT CHAIN FIXED (decoupled from the Computer Use master switch; the vision gate split to the describe leg; honest refusals — never the 1×1 capture), THE CONTEXT POPUP'S SECTIONED REBUILD (the 420px panes, both legs twins), THE WINDOW CONTROLS (visible chips + hover identities + the true restore glyph), THE GROUPED SETTINGS NAV + the Functionality relabel, THE MEMORY add/edit + discipline, and THE TERMINAL discovery + usage:stats. THE RECOVERY CHAPTER: a mid-round sandbox reset lost 8 unpushed commits — everything was re-implemented and pushed per-workstream; PUSH-AFTER-EVERY-VERIFIED-COMMIT IS NOW BINDING (the worklog's R98-recovery-1/2/3 entries hold the protocol). The review pass was the orchestrator's systematic self-review under the owner's rush directive (the hex audit, the keyframe registry, the tab-id stability, both full suites) — a full subagent review is queued for R99. Full pipeline: lint, both typechecks, root 3,651/3,651 (12 env-skips), agent-core 2,370/2,370, e2e 12/12, docs:check 213/0/0, the license audit clean at 247 deps, version ×4 at 0.96.0. The owner's TEST CHECKLIST lives in round-98.md §6.) The prior state: the eighth-walkthrough round, v0.93.0 → v0.94.0 → v0.95.0.
