...
**Last updated:** 2026-09-06 (round-68 DELIVERED — the computer-use overhaul, closing the owner's v0.67.0 live Windows field report; the WEB-BROWSE stack the owner confirmed 100% working is untouched; the round's code + docs are complete in the working tree — the push/CI/tag/release close-out is the session's remaining step): (1) **SCREENSHOTS LAND INLINE AT THE CAPTURE MOMENT** (the owner: "The screenshots were supposed to be shown properly when they were actually taken, not at the bottom in a dedicated section. When the screenshots were taken they should be shown at that specific time."): every `{type:"screenshot"}` SSE sideband frame becomes a `screenshot` WorkingEntry appended to the OPEN liveTurn's working array at frame arrival — right after the in-flight tool row that captured it — and WorkingSection renders the new `ScreenshotRow` INLINE between the rows (compact tile, lazy raster fetch, honest "expired" placeholder, click-to-enlarge dialog); the R67-D bottom strip, `LiveTurn.screenshots`, `LiveScreenshot`, and the cap-8 are GONE, and the debug full-turn export carries the `[screenshot captured by <tool>]` marker (does not bump tool numbering). (2) **THE MONITOR VANISHES FROM CAPTURES**: `SetWindowDisplayAffinity(hwnd, WDA_EXCLUDEFROMCAPTURE=0x11)` via raw user32 FFI in `src-tauri/src/mini.rs` — applied on window build AND re-asserted on every re-open (idempotent, self-healing) — the "Agent is using your computer" bar stays fully rendered on the owner's physical display yet drops out of every capture API (GDI CopyFromScreen = the agent's own capture path, Windows.Graphics.Capture, OBS, screen share), so agent screenshots show what is BEHIND the bar and vision never reads the indicator; corollary: the owner's own recordings won't show the bar either (global exclusion, documented as intended); pre-Win10-2004 hosts fall back honestly (non-fatal); constant pinned by a cross-platform cargo test; cargo unrunnable in this sandbox — API cross-checked against tauri v2.11.5/windows 0.61.3 + a structural replica compiled on the Playground; CI's windows cargo check is the compile gate. (3) **WINDOWS INPUT IS RAW SendInput**: the preamble's dead `LoadWithPartialName('System.Windows.Forms')` is REMOVED (the live trace proved it dead while the captures' own Add-Type worked — every type/key/scroll died capability_fail_closed TypeNotFound); the ONE U32 Add-Type now carries platform-correct INPUT structs + SendText (KEYEVENTF_UNICODE — no escaping class of bugs) + TapKey/Chord/ModsDown/ModsUp; `composeVkChord` + VK tables replace the SendKeys table (the win/meta key = LWIN works for the first time, incl. as a click/drag modifier); the select gestures, horizontal scroll, and modifier holds all ride SendInput; the measured ARGV guard (31,875 base64) refuses an over-long model payload (type/setValue/writeClipboard) BEFORE the spawn — the old sizing comment's "stays far below" claim was FALSE (the timeout clamp never capped the text). (4) **THE FOREGROUND GATE SELF-HEALS** (`withForegroundRetry` in dispatch.ts): a frontmost mismatch or a script-level FRONTMOST_MISMATCH now ACTIVATES the target (the escalated activate ladder — AttachThreadInput sequence → SW_MINIMIZE/SW_RESTORE trick → honest re-verify) and retries ONCE, for type/key per-repeat/clicks/scroll/drag/mouse-down/mouse-move/hold-key; the refusal text now tells the truth ("the automatic re-activation was already attempted and failed"). (5) **EDGE'S WEB TREE IS SEARCHABLE**: `PokeChromium` — WM_GETOBJECT (OBJID_CLIENT) to every Chrome_RenderWidgetHostHWND child BEFORE the UIA walk (+400 ms settle + a sparse-retry re-walk) in buildSnapshot; find_elements routes through it, so the Edge/Chromium web accessibility tree returns real links/buttons/inputs BY NAME and element targets replace the screenshot loop. (6) **FRAMES LIVE 30 S** (MAX_FRAME_AGE_MS 10 s → 30 s — 10 s was shorter than the vision roundtrip itself; every zoom-then-click pair died frame_stale) and **the vision relay RETRIES 429/5xx** (shared attempt body for both wire formats, two retries with [1500, 3000] backoff, 4xx terminal; test hooks exported). The prompts, the built-in computer-use skill and the tool descriptions teach the new truth (BROWSER CONTENT IS SEARCHABLE, CHAIN DISCIPLINE — act immediately on 30 s frames + verify with a small zoom crop, middle_click = new tab, the auto-activation honesty), and the golden fixture is regenerated. Verification: root **2003/2003 in 122 files** (was 1961/122; agent-core 1101/59, frontend src 890/61, e2e 12/12), lint/typecheck exit 0, build ✓, docs:check 0/0 at the round date (the sandbox clock reads 2026-09-05 vs the round date 2026-09-06 — proven green with the clock pinned, the R67-F precedent), version 0.68.0 ×4. Full detail in `docs/ui-iterations/round-68.md` · **What's next:** the owner's live Windows verification — the 7-step Edge flow is the acceptance test (searchable browser tree via find_elements, inline screenshot rows at their capture moment, SendInput typing/keys, the self-healing foreground, the capture-excluded monitor — the checklist in `runbooks/COMPUTER-USE.md`; all Windows behavior is construction-pinned, PowerShell/cargo never run in this sandbox) — then the standing queue: browser-wall self-bypass (future), packaging/code-signing (SmartScreen), ratings-driven prompt tuning, the temp-.ps1 move if the Windows preamble grows again (~1.8K argv headroom left) · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
