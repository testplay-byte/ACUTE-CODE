...
**Last updated:** 2026-08-30 (round-53 DELIVERED — the connection round; every fix traces to the owner's sixth Windows test session — the FIRST run of the packaged app): **THE STALE-PORT BUG** (the owner's exact report: "Could not reach agent-core at http://127.0.0.1:55963 (TypeError: Failed to fetch)" + "Agent core unreachable" in Settings + "pre saved api keys were not loaded" — one root cause, three symptoms: the sidecar binds an EPHEMERAL port per launch and the UI PERSISTED it to localStorage, so the next boot rehydrated a dead endpoint with demoData:false; inside Tauri the store now persists NOTHING — every boot starts from safe defaults and adopts the live endpoint in memory, and pre-R53 persisted blobs are retired on read (store version 2; pinned by test)); **THE HANDSHAKE RACE** (`sidecar_info` was answered ONCE at webview boot while the Rust handshake blocked `setup` for up to 25s — losing the race left the app on the dead port for the whole session; the handshake now runs on a BACKGROUND thread (the window paints immediately) and the webview POLLS via the new connect loop: sidecar_info every 400ms, deadline 90s, fail-fast on a shell-reported failure); **CONNECTION GATE** (ConnectionGate.tsx NEW — the whole app tree is gated on the engine: a branded splash while connecting so NO query can fire at a stale endpoint, and an offline screen with the REAL shell error + a one-click RESTART ENGINE button when it fails); **RESTART + WATCHDOG** (new shell commands `sidecar_status` {phase, port?, error?} and `restart_sidecar` — full teardown → fresh handshake, serialized, no app restart; a child monitor thread flags mid-session deaths and a 20s ping watchdog flips the UI to offline → restart → reconnect → ALL queries invalidated); **SIDECAR.LOG** (every lifecycle line — spawn command, ready, health, injected provider keys as id+length ONLY, failures, exits — appends to %APPDATA%\acute-code\sidecar.log, rotated at 1 MB: the packaged app finally has remotely readable diagnostics; `state_dir()` also fixes dev-on-Linux which used to hard-require APPDATA); **KEYS WERE NEVER LOST** (Credential Manager holds them exactly where the launcher seeded them — the screens just couldn't reach the engine that reports them; the spawn log now proves injection per boot). Verification: 1058/1058 tests (75 files; +23 incl. the stale-port regression, v1-blob retirement, adopt-after-N-failures, fail-fast, honest timeout, watchdog death + blip-reconnect, retry-through-shell), cargo check GREEN ON THE WINDOWS TARGET (a user-local mingw made x86_64-pc-windows-gnu checkable in-sandbox — every cfg(windows) path type-checked locally, CI re-verifies), and a LIVE browser battery on the real sidecar: fresh-profile wizard → **project created via the exact failing path** (r53-battery-project) → live chat turn replied exactly as asked (glm-5.2:free, 8.3s) → Usage on real ledger data → Sub-agents tab fully live with the model catalog. Version **0.53.0**. Full detail in `docs/ui-iterations/round-53.md` · **What's next:** the owner's Windows re-test (the packaged app must now either work or SAY WHY — sidecar.log + the offline screen make every failure self-explaining), installer code-signing (SmartScreen), the deepseek-harness future candidates, Files-tab polish · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
