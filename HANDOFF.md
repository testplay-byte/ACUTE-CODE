...
**Last updated:** 2026-09-11 (round-87 DELIVERED: the UX + capability round — the owner's improvement list, v0.84.0 → v0.85.0. Settings → About is NEW (version card + update check + the application-wide RESET: POST /system/reset wipes every table + reseeds factory state + clears the keyring + purges ~/.acute; the Tauri purge_provider_keys command clears Credential Manager first). credentials.txt is GONE from the launcher (the GitHub token is asked once + saved at .acute/github.pat, or ACUTE_GITHUB_PAT; provider keys are saved in-app only). ask_user shipped end-to-end (the mid-task question card — option pills + custom answers, batched, 10-min timeout; agent-question.requested/resolved session events; the resolve route; migration 0033) and the todo list now renders LIVE in chat (TodoCard via todo-updated frames). Models & Providers redesigned (bordered model cards + the R87 capability chips — migration 0032: PDF input + text/image/video/audio outputs + sizeLabel; reasoning + tool use REMOVED from the dialog per the owner's directive; configure-before-add: the picker opens the config dialog, never a direct add; the animated self-dismissing test button; the NVIDIA phantom-add fixed with hasKey-aware existingIds). The embedded browser: 1440×900 default, KEEP-ALIVE browser panels (no reload/glitch on tab switch; survives the new-tab menu), read_dom returns per-element x/y/w/h, screenshots are panel-only (the full-display fallback REMOVED). The chat/shell batch: adaptive scrollbars, chat min width halved, graduated composer shrink, selector trajectory gating + frosted-glass backdrop, the sidebar rail expands on project click. Full pipeline green: 157/2,839 frontend + 94/1,877 agent-core tests, lint/typecheck/build/e2e 12/12/license 134/smoke.) · **What's next:** the R86-era queue unchanged — Wave 2-b (the turn-loop harness, risk #1) + the remaining 51-route server.ts split, then Wave 3 (the frontend seams; deleting the dead sessions/ dir) · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
**Last updated:** 2026-09-10 (round-86 DELIVERED: the verification + SSE-extraction round — the owner's task-list session, v0.83.0 → v0.84.0. Three parallel sweeps re-verified every R85 claim (backend exact; four frontend numbers corrected; the dead sessions/ dir caught being maintained; three narrow frontend seams the 'zero' claim missed). THE SSE ROUTE EXTRACTED — `routes/sse.ts`, the final-phase domain of the R84 split: the 476-line streamed turn route moved verbatim (byte-identical round-trip verifier), server.ts 2,439 → 1,913, eleven single-consumer imports pruned, registration order preserved, the full pipeline green (2,825 tests byte-identical to baseline, e2e 12/12 on the built dist, live boot smoke). Ops: six stale branches deleted (only `main` remains; the r61 branch's superseded cua-helper line documented in round-86.md §4b with recoverable tips), the stuck v0.83.0 DRAFT release PUBLISHED (both assets verified), the DASHBOARD's plan section (two rounds stale, live on the public Pages) truth-synced. · **What's next:** the R87 scoping decision — Wave 2-b (the turn-loop harness, still risk #1) + the remaining 51-route server.ts split (the SSE domain done; the rest mechanical), then Wave 3 (the frontend seams) · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
