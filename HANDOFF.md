<!-- last-reviewed: 2026-08-30 round-51 -->
# ACUTE-CODE — Agent Handoff Document

**Last updated:** 2026-08-30 (round-51 DELIVERED — the desktop-shell-delivery round; every workstream traces to the owner's fourth Windows test session after R50 — he CONFIRMED the composer overall, the Access button, sub-agent live streaming, and the stats content, all frozen): **THE DESKTOP SHELL SHIPS** (the round's root-cause discovery: the owner's recurring "browser still not working" was never a browser bug — the launcher runs the app in the SYSTEM BROWSER where `isNativeBrowserAvailable()` is false, so he was ALWAYS on the R43 proxy iframe and the R50 native Chromium browser never activated; fixed by DELIVERING the shell: `scripts/release/stage-sidecar.mjs` stages a PROVEN-bootable sidecar tree (agent-core dist + pruned prod deps + vendored shared, 216 MB after win32 prune — pnpm deploy was evaluated and REJECTED with reproduced evidence), `sidecar.rs` gained `resolve_sidecar_command` (ACUTE_SIDECAR_CMD → bundled `<resource_dir>/sidecar/node.exe` + `app/dist/main.js` → dev repo-walk, byte-identical) + key slots 2–4 through Credential Manager (`openrouter-slot2/3/4`), `tauri.conf.json` bundles NSIS currentUser with `mainBinaryName: "ACUTE-CODE"` + window `minWidth 1000/minHeight 620`, release.yml gained a windows `desktop-installer` job (Node 24.20.0 pinned = the shipped runtime, rust cache, build → stage → `pnpm tauri build` → `ACUTE-CODE_<v>_x64-setup.exe` attached to the draft release alongside the kit), and the LAUNCHER went desktop-first on Windows (latest release → download → silent `/S` install → `cmdkey` Credential-Manager key seeding (main+slots, values never logged) → launch via the HKCU uninstall registry key; EVERY failure falls back to the untouched dev flow; `--web` forces it), plus BrowserPanel honesty — an engine badge "Chromium (native)"/"Proxy fallback" + native-failure auto-fallback to the proxy renderer with an explanatory card); **composer polish** (Add Context = paperclip only; model button = model id only; the provider→model flyout measures row+popover+viewport via the pure `computeFlyoutGeometry` and can no longer be cut off — `position:fixed` because the popover now scrolls (dropdown-in-scroller avoidance, documented); the donut lost its inline %, gained a ~220ms hover-bridge timer so the popover STAYS OPEN while moving into it, amber/red grading at 60%/85%, and the context report now returns `usage:{main,subagents,combined}` — subagents = direct children's usage_events, live-battery-proven; the toolbar clusters are shrink-0 + wrap so the composer can never overlap its controls); **sub-agent panel de-sloped** (the final report's accent left rail — the owner's "AI slope" — is GONE, the bubble is byte-identical to every other assistant message with a quiet tertiary label; the todo card shows the FULL checklist (items carried end-to-end from todo.update payloads, capped 50, per-item ✓/dot/○ glyphs, max-h-40 scroll) instead of just a progress bar; the stats footer is centered with hairline dividers); **main-agent efficiency** (prompts.ts: the EFFICIENCY section — understand-first with ONE batch of parallel reads, plan once, fewest steps, concise reasoning; the mandatory read-back-after-write and the "budget of 80, use it" step-incentive are GONE; the runtime gained the deepseek-harness-inspired LOOP-HYGIENE GUARD — `createLoopGuard()`: nudge at 3 identical consecutive calls, honest retryable stop at 5 identical/6 failed, `code: LOOP_GUARD` on turn.error, both sync+streamed paths, stops mid-stream; WorkingSection gained ToolIconChip — delegate_task + file-edit rows now visibly distinct WHEN COLLAPSED, no left rails); **the DASHBOARD Usage page** (`scripts/export-usage.mjs` → public-safe usage.json with hashed ids + redaction denylist → usage.html: overview cards, activity chart, tool leaderboard, model cards, collapsible projects → sessions with nested sub-agent runs; real data shipped: 15 projects/26 sessions/505K tokens; the owner regenerates from his own DB with `pnpm usage:export`); **`pnpm smoke`** (the owner's "a file which you run" — 320 curated critical-path tests in ~17s with a green/red verdict). deepseek-harness studied in docs/research/deepseek-harness-notes.md (MIT, ideas-only; top future candidates: compaction pressure-trigger, continuable sub-agent children, skill-catalog progressive disclosure). Version **0.51.0** across all four manifests + CHANGELOG. **978 tests green** (69 files; was 930/68 — +48 all additive; smoke 320/12-files; e2e 12/12; build green) + **live battery 6/6** (boot on the R51 tree; thinkingLevel=low streamed turn on the new prompt+guard tree; the usage split live; a REAL delegation completing end-to-end — the guard does NOT false-positive on healthy varied runs; the split widening after delegation; export-usage green on the live DB) — full detail in `docs/ui-iterations/round-51.md` · **What's next:** the owner's re-test of the DESKTOP APP itself (ACUTE.bat → install → launch; the engine badge must say "Chromium (native)"; SmartScreen will warn — unsigned installer), then the deepseek-harness future candidates, installer signing, Files-tab polish · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
