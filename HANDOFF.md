...
**Last updated:** 2026-09-02 (round-61 DELIVERED — the computer-use + extensibility round, ALL delivered per the owner's directives): (1) COMPUTER USE (the owner: "give the user the option to turn on and off the computer use") — the agent observes and actuates the REAL desktop GUI via a 30-tool surface (the uploaded 13-doc spec, code-complete): accessibility-tree-first observation (get_app_state) with screenshot-coordinate fallback, background-safe element actions, receipts-never-promises, 22 named self-teaching refusal codes, an ENFORCED kill switch (STOP → all further calls refuse kill_switch_active + held buttons physically released), a redacted audit journal at `<project>/.acute/computer-use/audit.jsonl`, and per-platform backends — Windows (PowerShell + UIA, no extra deps, postcondition-verified activation), macOS (osascript + TCC Accessibility/Screen Recording), Linux X11 (xdotool/wmctrl/scrot/xclip + AT-SPI via gdbus; Wayland raw input honestly unsupported); the whole surface is SETTINGS-GATED (master switch default OFF; observe posture registers only the 11 read-only tools; act posture's real-input risk classes ride the run_command approval channel in ask mode); (2) the SEPARATE VISION MODEL (the owner: "for the vision we are utilizing a separate model… the provider completely separately") — vision mode off/separate/main: "separate" = any provider + model + its OWN dedicated key slot (keyring pseudo-provider `<id>-vision` → credential ACUTE-CODE/provider/<id>-vision + env ACUTE_PROVIDER_<ID>_VISION at spawn; masked in the UI, never shown in full; honest primary-key fallback), "main" = the turn's model ONLY when its row has the new models.supports_vision flag (the eye toggle in the Computer Use tab flips it per row; migration 0023 adds the column + the skills + mcp_servers tables); the relay is fetch-direct chat-completions/anthropic-messages, invoked only for screenshot/zoom describe:true; (3) THE MONITOR (the owner: "in a mini window it will show the details and their stats while the agent is using computers… what it's about to do, how it's thinking, the progress") — the right-sidebar Computer tab (status chip OFF/STOPPED/LIVE/Idle, 4 stat cells, elapsed clock, the newest-first event ring with refusal-code chips, STOP) + the FLOATING draggable mini window (app-wide, 340px card: what it's doing / how it's going / STOP), both fed by per-execution {type:"computer-use"} SSE frames intercepted in stream-store BEFORE the liveTurn guard + polled GET /computer-use/session truth; (4) EXTENSIBILITY (the owner: "the ability to add multiple skills… the ability to add MCP servers too") — SKILLS (Settings → Skills: SKILL.md-style modules, progressive disclosure — name+description ride the system prompt, body via the new read_skill tool; built-in computer-use skill seeded, edit/disable-yes delete-no; user skills full CRUD) + MCP SERVERS (Settings → MCP: stdio JSON-RPC children with SANITIZED env — no ACUTE_PROVIDER_* leak ever; tools bridge as mcp__<server>__<tool>; probe + live tools list; fail-soft) + GET /plugins (12 built-ins + the live catalog + the external .mjs report); (5) the PROMPTS upgrade (the owner: "improve its tool calling skill using") — 3 tool-discipline rules + the honesty rule + the closing contract + gated SKILLS/COMPUTER-USE/MCP sections (golden fixture pinned). Verification: root suite **1491/1491 in 102 files** (was 1348/91; agent-core alone 786/786 in 48 files — re-run green by the docs agent 2026-09-02), lint/typecheck CLEAN, docs:check 0 failures; the 11 new R61 suites = 93 backend (computer-dispatch 33, computer-session 12, computer-errors-audit 15, computer-use-plugin 13, computer-vision 10, skills-mcp 10) + 50 UI (SkillsTab 11, McpTab 12, ComputerUseTab 12, ComputerPanel 8, ComputerMiniWindow 7). **HONEST CAVEAT: no platform backend has been live-verified on a real GUI (this sandbox is headless; dispatch tests inject a fake backend; plugin tests run the real Linux backend headless) — the LIVE-VERIFICATION CHECKLIST in docs/runbooks/COMPUTER-USE.md is the owner's first next-session item.** Version **0.61.0** (the version bump + commit/push/CI/DASHBOARD sync remain the orchestrator's close-out queue for this round). Full detail in `docs/ui-iterations/round-61.md` + the two new runbooks `docs/runbooks/COMPUTER-USE.md` + `docs/runbooks/EXTENSIBILITY.md` · **What's next:** the live verification checklist (per platform: master switch + Test readiness → list_apps → get_app_state → one act-mode approval action → the mini window + STOP), then the standing queue — the R50 items, multi-agent support (the owner's stated direction: "after this we will work on the improvements functionality and management": the smarter main agent with flex options, helper agents, agent self-browsing/testing of built web apps, automation tools, everything module/plug-in — the ratings loop feeding prompt tuning), packaging/code-signing · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
