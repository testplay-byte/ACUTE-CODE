...
**Last updated:** 2026-08-26 (round-43 DELIVERED — the owner's 9-item R42 verdict round: **CI GREEN again** (two real Rust E0599s fixed in `src-tauri/src/browser.rs` — `user_data_dir`→`data_directory` + `on_navigation` moved onto the builder, pinned tauri 2.11.5; first green since R38, runs watched to `success`; the audit's “YAML rot” was a sandbox display artifact — no yaml edits); the dead `stealth/ox-alpha` model RETIRED → typed 46-entry catalog (18 free, default `z-ai/glm-5.2:free`, migration 0013) + shared Free-only/All filter (Settings, chat picker, sub-agent picker) + OpenRouter **fallback chain** (`models:[model, openrouter/free]` — live-verified through a 429 storm); failed turns now **persist `turn.error`** + SSE terminal error frame + TurnErrorCard with Retry/Copy (silent death eliminated); chat geometry measured+fixed (w-full panel, 1080px centered column, no-floor sidebar cap — zero overflow 700–2560px); Sub-agents settings tab (temporary OpenRouter pool-key slots 2–4 + `orchestration.subagentModel`, tool-capable-validated); **embedded browser INSIDE the app**: sidecar proxy (per-tab `bt` ticket auth for iframes, HTML/CSS rewriting, framing headers stripped, private-net guard) + BrowserPanel rewrite (viewport presets 375→1920 + custom + zoom + rotate) + `browser_control` agent tool; SubAgentPanel phase-card redesign; sidebar session borders/depth/state icons; ClampedText 6-line; migration 0014 made `delegate_task` REACHABLE from seeded templates (it never was — sub-agent fan-out now works end-to-end live). **397 tests green** (389 unit + 8 e2e; was 262), license clean 131 deps — full detail in `docs/ui-iterations/round-43.md` · **What's next (owner verdict on R43, then the deferred queue):** security holes P0-3..P0-5 (deferred by owner), packaging/versioned releases, memory system v1, PTY terminal, real search API (Decision 1), revert UI + session search/fork per the audit's Track 2) · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
