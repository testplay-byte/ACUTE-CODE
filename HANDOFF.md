...
**Last updated:** 2026-09-02 (round-63 DELIVERED — the desktop-update round): (1) **THE ROOT CAUSE of "the desktop application was not reinstalled properly": rounds 61–62 shipped features but never pushed their `v*` release tags, so no installer existed beyond 0.60.0** — the site/dev flow served the tip while the owner's packaged app stayed frozen at 0.60.0 (which also explains "features do not work on the site": the embedded browser + computer use are desktop-only, and the desktop was stale). 0.63.0 IS tagged: the Release workflow builds the installer and opens the draft release, so the owner's next `ACUTE.bat` run updates 0.60.0 → 0.63.0 and verifies it. **The tag push is now a REQUIRED close-out step** (docs/runbooks/MAINTENANCE.md recipe g). (2) **THE LAUNCHER'S VERSION-TRUTH CHAIN**: every desktop launch checks THREE versions against the newest GitHub release — the uninstall registry's, the installed exe's real FileVersion ON DISK (new PowerShell probe), and after launch the running engine's `/health` version — a *hybrid* install (registry bumped, exe stale — a silent install racing a closing app) is **deleted completely and reinstalled** (NSIS uninstaller run synchronously via `_?=`, residue swept, stale registry entries dropped, processes polled-gone first instead of a fixed sleep), then verified again; failures set a sticky repair flag so the next run self-heals. Installer downloads are **sha256-verified** against GitHub's asset digest (one retry). New commands: **`ACUTE.bat reinstall`** (delete + fresh install + verify + launch) and **`ACUTE.bat uninstall`** (clean removal, no credentials needed, `%APPDATA%\acute-code` data always kept); `status` shows the full version truth incl. update-pending. (3) **`/health` tells the truth**: the engine's reported version derives from the package.json beside the compiled code (staged verbatim into the installer) — it had been a hardcoded "0.3.0" for 60+ rounds. (4) The site-flow plan panel now says plainly that the embedded browser + computer use need the desktop app. Verification: agent-core server tests green (VERSION now pinned to package.json), launcher logic verified by a 12-scenario simulation battery (current/hybrid/upgrade/newer-kept/forced-reinstall/repair-flag/engine-mismatch/missing-files/first-install/verify-retry/unrepairable-fallback/real-uninstall-on-temp-dir) + py_compile + a live `status` smoke; ACUTE.bat CRLF preserved (77 lines). **HONEST CAVEATS: the launcher's Windows-only paths (FileVersion probe, uninstaller orchestration, process wait) are code-complete + simulation-verified but live-verify on the owner's machine — the first `ACUTE.bat app` run after 0.63.0 is that verification; if anything misbehaves, `ACUTE.bat reinstall` forces the full path and `status` shows the version truth.** Version 0.63.0. Full detail in `docs/ui-iterations/round-63.md` · **What's next:** the live-verification checklist (the desktop update 0.60.0→0.63.0 itself, computer-use + agent-browser native paths on the real desktop), then the standing queue — multi-agent support, packaging/code-signing (SmartScreen), ratings-driven prompt tuning · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
