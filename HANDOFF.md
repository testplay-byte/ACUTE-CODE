...
**Last updated:** 2026-08-30 (round-52 DELIVERED — the command-supervision round; every workstream traces to the owner's fifth Windows test session): **BACKGROUND JOBS + THE HANG FIX** (the owner's exact report: `start /B node server.js > server.log 2>&1` showed "running…" for 10+ minutes and the agent never checked — root cause: Node's `close` only fires when the stdio PIPES close, and a `start /B` grandchild inherits the pipe handles for its whole lifetime; `run_command` now tracks `exit` and `close` SEPARATELY: a pipe-holding grandchild registers a BACKGROUND JOB and the call resolves IMMEDIATELY with the job id + polling instructions, a never-exiting shell is TREE-KILLED by a 60s hard watchdog; NEW tools `job_status`/`job_stop` + REST `GET /jobs`, `GET /jobs/:id`, `GET /projects/:id/jobs`, `POST /jobs/:id/stop`; the registry tails the orphaned pipe + the parsed `> logFile`, stops via tree-kill/pgrep/PowerShell, caps at 100 jobs/12h; LIVE battery found + fixed the Unix `&` gap: a fully-redirected `cmd > log 2>&1 &` closes its pipes instantly and USED TO vanish from tracking — background launches now spawn `detached: true` (POSIX process-group leader) and register a DETACHED job whose liveness is the sync `kill(-pid, 0)` group probe, `job_stop` kills the group; LIVE TERMINAL OUTPUT: run_command streams batched `tool-output` SSE frames while running, rendered as a live tail in the chat working section / expanded detail / sub-agent panel); **SUB-AGENTS STOPPABLE + SUPERVISED** (lib/turn-registry.ts NEW — the server's activeTurns map moved to a shared registry where the orchestrator registers each running CHILD, so `POST /sessions/:id/stop` aborts a sub-agent exactly like a main turn (reason "owner"); the parent gets an honest "STOPPED BY THE OWNER" report; the child supervisor samples every 15s (childWatchdogMs) emitting `subagent-status` frames with a `watch` payload — lastActivity/lastEventAgeMs/toolCount/todosDone/Total/elapsedMs/stalled — and a child silent for 5min (childStallTimeoutMs) is aborted with reason "stall" + an honest parent report; SubAgentPanel: a Stop button on the header (guarded on the polled detail), a stopping chip, the WatchLine (amber when stalled), the `detail` strip ("stopped by the owner"/"stalled — …"); the two knobs live in Settings → Sub-agents); **THE REAL USAGE SCREEN** (the in-app /usage was still PlaceholderPage — the owner called it out; `GET /usage/detailed?days=` = a faithful port of export-usage.mjs's aggregation with a PRIVATE shape (raw ids, no redaction — only the public export redacts); UsageScreen.tsx: stat cards, a 7/14/30/90-day activity chart, tool leaderboard with failures, model cards, and a projects → sessions drill-down with sub-agent children NESTED under their parent + chat deep-links); **THE PLUGIN TOOL SYSTEM** (ADR-0025: every built-in tool group moved VERBATIM into tools/plugins/*.ts behind tools/registry.ts's PluginDefinition; buildProjectTools assembles through the registry with IDENTICAL semantics (ADR-0019 allowlist, memory switch, delegation gating); the catalog is COMPUTED from the real declarations — the R44 TOOL_CATALOG drift is structuralized away; EXTERNAL .mjs plugins from ~/.acute/plugins (default) and <project>/.acute/plugins (opt-in `tools.externalPlugins`), name grammar + caps 32/64 + built-in-wins collisions + fail-soft; the vitest dynamic-import trap solved: native `import()` → `createRequire` require(esm) fallback, probe-verified); **model-flyout hover bridge** (the provider row's instant mouseleave closed the flyout ~18px away before the pointer arrived — the R51-c 220ms grace timer now bridges it); **CORS [::1]** (the IPv6 literal origin missed the allowlist — every preflight 401'd and the webapp silently fell back to demo data). Version **0.52.0** across all four manifests + CHANGELOG + ADR-0025 + round-52 report. **1035 tests green** (73 files; was 978/69 — +57 all additive) + **live battery 6/6 incl. the browser pass** (the owner's Unix-shaped trap reproduced THEN verified fixed live; the agent following the job_status discipline unprompted; the group-stop; a real delegation with heartbeat frames; a mid-flight child stop end-to-end; agent-browser: the Usage screen on real data, the Terminal Background-jobs section with a working UI Stop, and the SubAgentPanel Stop → "stopped by the owner") — full detail in `docs/ui-iterations/round-52.md` · **What's next:** the owner's Windows re-test (the background-jobs flow with a REAL dev server via `start /B`, the sub-agent Stop, the Usage screen on his DB), the v0.52.0 tag ships the FIRST release with the NSIS installer attached (the launcher's desktop flow finally has something to download), then the deepseek-harness future candidates, installer signing, Files-tab polish · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
