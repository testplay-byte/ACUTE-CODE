...
**Last updated:** 2026-09-08 (round-78 DELIVERED — THE HONEST-ERRORS + QUEUE ROUND, the owner's four verbatim asks, all implemented + unit-tested + live-battery-verified + browser-verified): (1) ERROR-CHAIN HONESTY (owner: "no matter the real cause, the UI always shows 'rate-limited'. Only a REAL rate limit may show as one; every other failure must show the API's ACTUAL error text. Robustify the whole error chain"): the AI SDK's RetryError is now UNWRAPPED before classification — NEW unwrapRetryError (AI_RetryError → lastError/errors) + extractStatus walking lastError/errors (depth ≤ 4, cycle-guarded) so a wrapped 429 classifies by STATUS instead of message-pattern luck; a 403 whose real body says region/moderation/permission/unavailable reclassifies to fail-fast unknown carrying the REAL text (a region block no longer lies "authentication failed"; plain 403 stays auth, 401 stays status-only); userMessage/classMessage are the provider's REAL words (240-char cap, the generic lines demoted to the empty-text fallback), and providerError rides every meta.retry frame + the 502 envelope + the persisted turn.error — the LIVE RetryStatusCard renders the class chip + the API's actual text (mono, 3-line clamp, the R77 expand toggle for > 240 chars); (2) THE COMPOSER ACTION ANCHOR (owner: "the chat bottom buttons occasionally render in wrong positions"): the toolbar is now a WRAPPING area (left cluster + ml-auto selectors) plus THE ANCHOR (data-composer-actions, shrink-0, justify-between sibling, NEVER wraps) holding Continue/Send/Stop/queue-send pinned bottom-right — the action button can never jump lines (browser-verified stable at 386/480/560/641/900 px); (3) RETRY SETTINGS (owner: "a retry-config section in General Settings — switches for auto-retry on timeout / rate limit"): RetrySettings {autoRetryRateLimit, autoRetryTimeout, autoRetryNetwork} (all default true) behind GET/PUT /settings/retry, the ladder gated PER CLASS in both turn paths (a disabled class fails fast: attempts 1, no meta.retry frames, the real text), rendered as the Auto-retry card above Debug mode on the Settings tab now LABELED General (URL id 'advanced' unchanged); (4) THE MESSAGE QUEUE (owner: "while the agent is working, the user can still send messages; they queue and auto-send right after the current tool call; the agent reads them with full context and continues — never interrupting the in-flight flow"): a session_events row of NEW type message.queued (the exact message.user payload; delivery is a strict type flip so the message lands where it was QUEUED), three delivery paths — loop-top flip before assembleHistory (the model reads the queued text WITH the completed tool results; a ladder wait delivers at the next rung because the retry re-enters the loop top), the turn-end same-stream continuation loop (meta.queue_continue, the first queued event consumed into the next turn's args, the rest flipped, task_complete per turn, the debug analyst + done frame ONCE, capped at 25, one registerTurn + abort controller across the whole loop, Stop leaves the rest queued), and the pre-flip crash/stop recovery (both turn functions deliver lingering queued messages BEFORE the next send's user event) — plus POST /sessions/:id/queue (409 NO_LIVE_TURN without a live turn; the user.queued frame bridges onto the open stream via the turn registry's new notify hook) + DELETE /sessions/:id/queue/:seq, and the frontend (the queue-send button beside Stop, amber chips with X-remove + Send-now, delivered messages as user bubbles, the queued fold item, the NO_LIVE_TURN fall-through). Verification: root 2708/2708 in 144 files (agent-core 1742 + frontend 954 + e2e 12; +81 over R77), lint clean, typecheck clean, scripts/battery-r78.mjs 12/12 (a dedicated sidecar + a mock OpenRouter-compatible provider that records request bodies as the model-facing oracle: honest bad-model error, real 429 ladder with the real text, queue-during-ladder-wait, retry-gate fail-fast, mid-turn delivery, same-stream continuation, crash recovery, route honesty), browser-verified on the real dev stack (the real account-wide 429 storm, the chip→bubble delivery, Stop mid-ladder stays retryable, the settings toggle persists server-side, the anchor at 5 widths). **R77 CLOSED: v0.76.0 = commit 6e2d55f, tag pushed, remote synced.** The R78 release close-out (commit → tag v0.77.0 → Release → publish → CI watch) follows per MAINTENANCE recipe g — the release agent owns it. · **What's next:** the R79 queue: delegate_task task_id/background/resume (the standing R73 deferral), external plugin ctx enrichment (cline's appendContext seam), lessons-ledger as the reminder injector's third consumer, ratings-driven prompt tuning, the optional v0.68.0 backfill · **Maintained by:** the orchestrator agent · **Audience:** any AI agent (or human) taking over development
...
7. `docs/decisions/` — ADRs 0001–0025 (never renumber; continue at 0026+).
...
## 3. Exact current state (2026-09-08, round-78 DELIVERED — v0.77.0 staged, release close-out pending)

**R78 is DELIVERED: the owner's four asks (honest errors, the composer anchor, retry settings, the message queue) shipped, unit-tested (2708 in 144 files: agent-core 1742 + frontend 954 + e2e 12), live-battery-verified (scripts/battery-r78.mjs, 12/12), and browser-verified against the real dev stack — see `docs/ui-iterations/round-78.md`.** R77 is CLOSED (v0.76.0 = commit `6e2d55f`, tag pushed, remote synced). **The minimum commit to verify from is `6e2d55f`** — the R78 work (agent-core + src + tests + battery-r78.mjs + this round's docs + the 0.77.0 version bump) sits uncommitted ON TOP of it; the release agent commits it, tags v0.77.0, pushes, and watches CI (status.json's ci field is the placeholder it syncs). The round-28-era table below is kept as the historical record.

**The next queue (R79+):** delegate_task task_id/background/resume (the standing R73 deferral — the orchestrator round), external plugin ctx enrichment (cline's appendContext seam), the lessons-ledger affordance as the reminder injector's third consumer, ratings-driven prompt tuning, the optional v0.68.0 backfill tag, and the standing items (edit-linting, installer code-signing, the Files-tab polish, agent web-app-testing tools). Known R78 residuals for the next agent: the hard-reload caveat (the Stopped card + Continue live in the live stream store — a hard page reload renders from the folded log and the affordance ends; SPA navigation preserves it), and the sync/sub-agent path has queue PRE-FLIP only (no interactive queueing — children have no owner composer).
...
│   ├── decisions/0001–0025 + TEMPLATE.md           ADRs (sequential, never reused)
