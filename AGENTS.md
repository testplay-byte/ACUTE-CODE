...
- **Behavior.** `docs/runbooks/AI-AGENT-PLAYBOOK.md` (R126) is the normative behavior contract for every AI agent on this project — the reading order, the behavior laws (todo discipline, one-surface-at-a-time, only-fix-what-is-certain, honest reporting), the session arc, the sub-agent wave protocol + review gate, the gate stack, live verification, and the release ritual. A cold-start agent reads it before touching anything.
- **Sub-agent protocol (owner revision, 2026-08-23; wave mode added R126).** Sub-agents are an OPTIONAL tool, never a default. Dispatch one only when the task genuinely warrants it — e.g. large parallel research sweeps or exploration across many files — and the orchestrator first checks whether doing the work inline would be simpler and safer. Implementation work is normally done inline by the orchestrator; do not force dispatches, do not hand off work a sub-agent doesn't need to do. When a sub-agent IS used, it returns: what it did, artifacts, open questions; review failures go back to the producer. R126 revision: for owner-directed large multi-surface efforts the orchestrator MAY delegate implementation to review-gated sub-agent waves (one surface per wave, full brief + strict diff review per the PLAYBOOK §4) — proven zero-defect on the twelve-wave R126 redesign; the gates and review discipline are what make it legal.
