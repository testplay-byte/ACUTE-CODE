...
|---|---|---|
| `src/server.ts` | Fastify 5 app: `GET /health` (no auth), bearer-token wall on every other route (API.md §1), agents CRUD per API.md §4, providers per API.md §8 (`GET/POST /providers`, `GET /providers/:id/models`, `POST /providers/:id/test` connection probe), single-agent sessions per API.md §5 (`POST/GET /sessions`, `GET /sessions/:id`, `POST /sessions/:id/messages`); uniform error envelope; `startServer()` binds loopback, prints the `ACUTE_READY {"port":…}` ready line | real (Waves 1–2) |
| `src/storage/db.ts` | `openDatabase(path)`: better-sqlite3 handle with WAL + `synchronous=NORMAL`, numbered SQL migrations applied transactionally and recorded in `schema_migrations`, one-time template seeding plus built-in provider seeding (openrouter) | real (Wave 1) |
...
| `src/storage/providers.ts` | Provider registry rows (list/get/create, reserved built-in ids, id slugification, idempotent built-in seeding at every open — SPEC §F4: openrouter exists before any request) — rows never contain keys | real (Wave 2) |
...
| `src/providers/registry.ts` | `ProviderKeyring` (in-memory snapshot of `ACUTE_PROVIDER_<ID_UPPER>` env vars — keys NEVER touch SQLite or any response), model-catalog fetch (`{baseUrl}/models`, 5-minute in-memory cache, sanitized `ProviderFetchError`), and `testProviderConnection` (keyed `/models` probe behind `POST /providers/:id/test`; bypasses the cache, scrubs errors) | real (Wave 2) |
