...
- [`runbooks/EXTENSIBILITY.md`](runbooks/EXTENSIBILITY.md) — **the four extension surfaces (R61, + the R66 + R70 + R71 addenda)**: external .mjs plugins, skills (SKILL.md-style, progressive disclosure — DB rows AND files on disk since R70: .acute/skills + ~/.agents/skills, 12 builtins with trigger-rich descriptions since R71, sticky bodies, the agent.skills allowlist), MCP servers (stdio JSON-RPC, sanitized env), and the built-in plugin catalog — with working examples of each
