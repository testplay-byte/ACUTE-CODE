...
| `orchestration/` | Session engine for the three run modes (ADR-0001): run coordinator, typed pub/sub message bus (metagpt), Kanban task-board state, `delegate_task` tool implementation (kilocode/openhands delegation-as-tool: child gets isolated context, returns a summary), topology-as-data loader (roles/wiring stored as JSON, not code), global 5-slot runner semaphore with queueing (ADR-0011). **R79 (ADR-0028, shipped in `agents/orchestrator.ts`):** the tool's addressable tier — `task_id` addressing (`sessions.delegate_task_id`, migration 0028), background fire-and-forget runs that outlive the parent turn (the immediate receipt + best-effort emit + the forwarded abort cascade), `resume` wait/collect/retry, the `delegation.collected` event, and the per-turn BACKGROUND TASKS reminder section as the delivery channel. |
...
| `memory/` | **R117-b (the real surface):** the `memory` SQLite table with TWO scopes — project memory (per-project rows, `memory_save`/`memory_recall`/`memory_list` tools, the per-turn ## Project memory digest) and WORKSPACE memory (cross-project rows, `/memory/workspace` REST, the ## Workspace memory digest composed above it) — plus `agents.memory_policy` LIVE (none = no digest+tools; on-start = first-turn-only), compaction summaries auto-persisted as project memory (the episodic bridge), the `session_recall` episodic tool, and CLI cwd project binding. The Hermes-style per-agent markdown files (MEMORY.md/USER.md + FTS5 mirror) remain the documented ROADMAP (API.md §4.8-4.11) — never implemented. |
...
| `main.ts` | Process entry: requires env-borne `ACUTE_TOKEN` + `ACUTE_DB_PATH` (exits if missing), opens SQLite, binds 127.0.0.1 on an ephemeral port, prints the one stdout ready line the shell parses (`ACUTE_READY {port}`, §2.1). Provider keys arrive as `ACUTE_PROVIDER_<ID>` env vars held only in memory (§7.3). R98-K: `startServer` (server.ts) additionally writes the `<dbDir>/acute-portal.json` discovery file — port + token for the CLI harness, user-local, 0600, removed on graceful close; the token is NEVER logged (stdout stays port-only). |
...
| **Sessions & events** | `sessions`, `session_events` | `sessions`: `id, project_id, mode, status, title, topology (JSON), created_at, ended_at` — plus the shipped delegation columns `parent_session_id`/`sub_role` (migration 0008) and, since R79, `delegate_task_id` (migration 0028: the parent model's own address for a delegated child, NULL = unaddressed; indexed with `parent_session_id` for the duplicate/cap/reminder lookups). `session_events`: **append-only** — `id (autoincrement PK), session_id, seq (per-session monotonic), ts, type, agent_id?, task_id?, payload (JSON)`. Transcripts, tool calls, approval steps, task moves, interruptions are all event types. Replay = SELECT ordered by `seq`. No in-place transcript mutation, ever (openhands; synthesis §6). |
