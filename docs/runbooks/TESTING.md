...
**Current counts (R70, verified 2026-09-06 by re-running the suites):**
the root `pnpm test` = **2177 tests in 126 files, all green** (measured
with the fresh `agent-core/dist` present — the 12 sidecar-e2e run
IN-SUITE against the built dist; without a dist they report as
env-gated skips: 2165 passed + 12 skipped. agent-core alone =
**1275/1275 in 63 files** (+95 this round), frontend `src/` alone =
**890/890 in 61 files** (unchanged — R70 touched zero frontend
files).
