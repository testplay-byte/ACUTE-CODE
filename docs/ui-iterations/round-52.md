...
### Also fixed in the second CI run (two REAL Windows behaviors the sandbox could never show)

- **The watchdog's tree-kill masquerade.** On Windows `taskkill /T /F` actually kills the whole tree → the pipes close → the `close` event beats the watchdog's 800ms follow-up → the normal-completion path resolved the command as a plain failure (`[exit code: 1]` — the exit code of a FORCE-KILLED shell, meaningless) with no hint that WE killed it or why. The `watchdogFired` flag now makes the close handler defer to the shared `finishTimeout` — a watchdog-killed command ALWAYS resolves as `[timeout]` with the kill note, on both orderings. (On POSIX the old code only worked by accident: `killTree` there kills just the shell pid, the pipe-holding grandchild survives, `close` never fires, and the watchdog wins by default.)
- **node's `stdio:'inherit'` chain does NOT hold cmd.exe's pipes on Windows** — the pipe-holder fixture (node spawns node with inherit) registered NO job there (the pipes closed with the shell). The owner's EXACT idiom does: the Windows fixture is now `start /B node holder.js … > holder.log 2>&1` — the same mechanism as his original 10-minute hang, i.e. the honest reproduction of the bug on the platform where it was reported. The fixture is platform-split: POSIX keeps node-spawns-node (same mechanics, different shell).
- **`start /B` passes the pipe HANDLES but not stdout (console-less runners).** CI run 2 proved the nuance: the job registered and stayed RUNNING (the pipe write handle was inherited — the hang mechanism) but the started process's stdout was NOT the pipe, so the live tail stayed empty. The fixture's holder.js therefore ALSO appends its late line straight to `holder.log` — the assertion rides the LOG tail on Windows (the owner's redirect idiom) and the PIPE tail on POSIX, both through the same job_status output.
