...
/** ROUND-115 (R115-E1): the QR tile + the fullscreen magnifier's shared
 * target size — one line, the same value both surfaces quote. The dialog
 * tile grows to fill the dialog (capped at 60vh so short windows still fit
 * the PIN + countdown + manual fallback below it); the fullscreen overlay
 * shows the code at arm's length (90% of the smaller viewport axis). */
const QR_DIALOG_SIZE = "min(60vh, 420px)";
const QR_FULLSCREEN_SIZE = "min(90vh, 90vw)";

/**
 * The QR CANVAS — the shared async renderer BOTH tiles wrap: the payload
 * string → renderQrSvg → the encoder's own SVG markup (module paths only —
 * generated by the qrcode library from the payload string, never user
 * HTML), on the fixed light ground (scannability beats theming: dark
 * modules on a light background regardless of the app's dark mode —
 * LINKING-PROTOCOL §2). The wrapping tile owns the size + chrome.
 */
 ...
 * rendered through the SVG encoder on a fixed light ground. `data-payload`
 * carries the exact string handed to the encoder (the phone parses this
 * payload; nothing is reformatted or dropped).
 * ROUND-115 (R115-E1): the tile grew from the 224px box to fill the dialog
 * (min(60vh, 420px)) and became the fullscreen magnifier's trigger — click
 * (or Enter; it is a real button now) to open the arm's-length view. */
function QrTile({
  payload,
  dimmed,
  onOpen,
  ...
  payload: string;
  dimmed: boolean;
  /** Opens the fullscreen magnifier; null = no live window (expired) — the
   * dead tile is not a button (a magnified dead code teaches nothing). */
  onOpen: (() => void) | null;
}) {
  return (
    <div
      ...
      role={onOpen !== null ? "button" : undefined}
      tabIndex={onOpen !== null ? 0 : undefined}
      aria-label={onOpen !== null ? "Show the pairing code fullscreen" : undefined}
      onClick={onOpen ?? undefined}
      onKeyDown={(e) => {
        if (onOpen !== null && (e.key === "Enter" || e.key === " ")) {
          e.preventDefault();
          onOpen();
        }
      }}
      className="rounded-md overflow-hidden grid place-items-center shrink-0 transition-opacity cursor-zoom-in outline-none focus-visible:ring-2 focus-visible:ring-offset-2"
      style={{
        width: QR_DIALOG_SIZE,
        height: QR_DIALOG_SIZE,
        background: QR_MODULE_LIGHT,
...
function FullscreenPairQr({ payload, onClose }: { payload: string; onClose: () => void }) {
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        // Capture phase on window: this runs BEFORE the document-level
        // listeners, so stopping here keeps the dialog open behind us.
        e.stopPropagation();
        onClose();
      }
    };
  ...
  const swallowPointer = (e: ReactPointerEvent | ReactMouseEvent): void => {
    // The dialog's outside-click dismissal listens at the document level —
    // a pointerdown on this overlay is "outside" it. Stopping the bubble
    // keeps the dialog open; the overlay's own onClick still fires (click
    // is a separate event).
    e.stopPropagation();
  };
  return (
...
/**
 * ROUND-115 (R115-E1): the small per-block copy affordance — the manual
 * fallback's address/PIN lines get one-button copies (the phone's
 * smart-paste flow consumes exactly these strings). The AgentChatPanel
 * CopyButton idiom verbatim: 24px hit target, hover:bg-hover wash (the
 * CSS-var leg), Copy → Check (success) for ~1.2s.
 */
function MiniCopyButton({ text, label, testId }: { text: string; label: string; testId: string }) {
  const styles = useThemeStyles();
  const resetAfter = useTimeoutClear();
    ...
    <button
      ...
      onClick={() => {
        void navigator.clipboard?.writeText(text).then(() => {
          setCopied(true);
          resetAfter(() => setCopied(false), 1_200);
        });
      }}
      aria-label={label}
      title={label}
...
function PairingDialog({
  ...
  // R115-E1: the fullscreen magnifier (click the QR tile), the copy
  // button's transient "Copied ✓" state, and the quiet clipboard-fallback
  // note (legacy select-text behavior — the manual blocks stay selectable).
  const [qrFullscreen, setQrFullscreen] = useState(false);
  const [copiedText, setCopiedText] = useState(false);
  const [copyNote, setCopyNote] = useState<string | null>(null);
  // R115-E1 (b): the fullscreen magnifier's stable close callback (keeps the
  // overlay's Esc listener subscription single-shot across renders).
  const closeQrFullscreen = useCallback(() => setQrFullscreen(false), []);
  ...
  // port, certFP, machineId, pin, ttl, expiresAt, + relay/machineLabel on
  // R112/R115 sidecars).
  ...
  // path); when the clipboard is MISSING or refuses, fall back to the
  // legacy select-text behavior — open the manual section (its lines are
  // selectable by design) + a quiet note. Never a hard error.
  const copyPairingText = () => {
    if (pairingText === null) return;
    const clipboard = navigator.clipboard;
    if (clipboard === undefined || typeof clipboard.writeText !== "function") {
      setManualOpen(true);
      setCopyNote("Clipboard unavailable — select the details below instead.");
      resetAfter(() => setCopyNote(null), 4_000);
      return;
    }
    void clipboard.writeText(pairingText).then(
      () => {
        setCopiedText(true);
        resetAfter(() => setCopiedText(false), 1_200);
      },
      () => {
        setManualOpen(true);
        setCopyNote("Copy failed — select the details below instead.");
        resetAfter(() => setCopyNote(null), 4_000);
      },
    );
  };
        ...
        {state.kind === "starting" ? (
          ...
          <div className="flex flex-col gap-4 px-5 py-4 overflow-y-auto" data-testid="pair-body">
            ...
            <div className="flex flex-col items-center gap-2.5">
              <QrTile
                payload={qrPayloadStr}
                dimmed={expired}
                onOpen={expired ? null : () => setQrFullscreen(true)}
              />
              {expired ? (
                <p
                  className="text-[11px] font-medium"
                ...
                >
                  <div>
                    <div className="flex items-center justify-between gap-2 mb-1">
                      <div className="text-[10px] font-medium uppercase tracking-wider" style={{ color: styles.textTertiary }}>
                      ...
                      {payload.addrs.length > 0 && (
                        <MiniCopyButton
                          ...
                          label="Copy the address and port"
                          testId="pair-manual-addrs-copy"
                        />
                      )}
                    </div>
                    <div
                  ...
                  <div>
                    <div className="flex items-center justify-between gap-2 mb-1">
                      ...
                      <MiniCopyButton text={payload.pin} label="Copy the pairing PIN" testId="pair-manual-pin-copy" />
                    </div>
                    <div
            ...
            {/* The clipboard-fallback note (quiet, one line, auto-clears) —
                only the copy path ever sets it. */}
            {copyNote !== null && (
              <p
                className="text-[11px] leading-relaxed"
                style={{ color: styles.textTertiary }}
                data-testid="pair-copy-note"
              >
                {copyNote}
              </p>
            )}
